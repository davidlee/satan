;;; satan-block.el --- SATAN-owned org-block find/replace -*- lexical-binding: t; -*-
...
;;   #+end_satan
;;
;; `satan-block-replace' finds the unique pair where the params line has
...
(defun satan-block--ts-now ()
  (format-time-string "[%Y-%m-%d %a %H:%M]" nil))

(defun satan-block--find-all (block-name)
...
and `:owner SATAN'."
  (let (matches)
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward satan-block--begin-re nil t)
        (let* ((begin-pt (match-beginning 0))
               (params   (match-string 1))
               (after-begin-line (line-end-position))
               (block-p  (satan-block--param params "block"))
               (owner-p  (satan-block--param params "owner")))
          (when (and (equal block-p block-name)
                     (equal owner-p "SATAN")
                     (re-search-forward satan-block--end-re nil t))
            (push (list begin-pt
                        (1+ after-begin-line)
                        (match-beginning 0)
                        (match-end 0)
...
(defun satan-block--render-begin-line (params)
  ...
  (let* ((stripped
    ...
    (concat "#+begin_satan "
            ...
            ":updated " (satan-block--ts-now))))

(defun satan-block-replace (file block-name content)
...
On `ok' the file's `:updated' parameter is refreshed."
  (with-temp-buffer
    ...
    (let ((matches (satan-block--find-all block-name)))
      (cond
       ...
       (t
        (cl-destructuring-bind (begin-pt body-begin body-end _end-pt params)
          ...
          (insert (satan-block--render-begin-line params)))
...
(defun satan-block-create-at-end (file block-name content)
  ...
  (with-temp-buffer
    ...
    (insert "#+begin_satan :block " block-name
            " :owner SATAN :updated " (satan-block--ts-now) "\n")
    (insert (if (string-suffix-p "\n" content) content (concat content "\n")))
    (insert "#+end_satan\n")
    (let ((coding-system-for-write 'utf-8)
          (tmp (concat file ".tmp")))
