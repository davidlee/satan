;;; satan-tools.el --- Tool registry + dispatch -*- lexical-binding: t; -*-
...
(defvar satan-tools nil
  "Alist of (NAME . SPEC) tool registrations.")

(defun satan-tool-register (spec)
  "Register or replace tool SPEC keyed by its `:name'."
  (let ((name (plist-get spec :name)))
    (setq satan-tools
          (cons (cons name spec)
                (cl-remove name satan-tools :key #'car :test #'equal)))))
...
(defun satan-tool--plist-like-p (val)
...
(defun satan-tool--validate-arg (args key constraints)
...
Supported constraints:
  ...
  (let* ((sym (intern (concat ":" (symbol-name key))))
    ...
    (cond
     ...
     ((null val) nil)
     (t (satan-tool--validate-value val constraints (symbol-name key))))))

(defun satan-tool--validate-value (val constraints label)
...
re-check `:required'."
  (let ((type    (plist-get constraints :type))
    ...
    (cond
     ...
     ((and (eq type 'object) (not (satan-tool--plist-like-p val)))
     ...
     ((and (eq type 'object) shape)
      (let ((cursor shape) err)
        (while (and cursor (null err))
          (setq err (satan-tool--validate-arg val (car cursor) (cadr cursor)))
          (setq cursor (cddr cursor)))
        err))
     ((and (eq type 'array) items)
      (let* ((item-constraints (satan-tool--items-constraints items))
             (idx 0) err)
...
(defun satan-tool--items-constraints (items)
...
(defun satan-tool-validate-args (spec args)
  ...
  (let ((schema (plist-get spec :args-schema))
    ...
    (while (and schema (null err))
      (let ((key (car schema))
        ...
        (setq err (satan-tool--validate-arg args key constraints))
        (setq schema (cddr schema))))
    err))

(defun satan-tool--capability-denied-p (spec run-ctx)
...
(defun satan-tool-dispatch (call mode-tools run-ctx)
...
declared `:capability' is absent."
  (let* ((id   (plist-get call :id))
         ...
         (spec (satan-tool-lookup name)))
    (cond
     ((null spec)
      (list :type "tool_result" :id id :ok :false
            :error (format "unknown tool: %s" name)))
     ((not (satan-tool-allowed-p name mode-tools))
      (list :type "tool_result" :id id :ok :false
            :error (format "tool not allowed in this mode: %s" name)))
     ((let ((cap (satan-tool--capability-denied-p spec run-ctx)))
        (when cap
          (list :type "tool_result" :id id :ok :false
                :error (format "capability denied: tool %s requires %s"
                               name cap)))))
     (t
      (let ((schema-err (satan-tool-validate-args spec args)))
        (if schema-err
...
(defun satan-tool--description (name)
  "Return the model-facing description for tool NAME.
Reads `<satan-tools-descriptions-dir>/<name>.md'; signals if
...
(defun satan-tool--jsonschema-type (sym)
...
(defun satan-tool--pattern-to-jsonschema (pattern)
...
(defun satan-tool--args-schema-to-jsonschema (args-schema)
  ...
  (let ((props nil)
    ...
    (while cursor
      (let* ((key (car cursor))
             ...
             (prop (cond
                    ((and (eq type 'object) shape)
                     (satan-tool--args-schema-to-jsonschema shape))
                    ((eq type 'array)
                     (let ((p (list :type "array")))
                       (when items
                         (setq p (plist-put p :items
                                            (satan-tool--items-jsonschema
                                             items))))
                       p))
                    (t (list :type (satan-tool--jsonschema-type type))))))
        (when enum
          (setq prop (plist-put prop :enum (vconcat enum))))
        (when pattern
          (setq prop (plist-put prop :pattern
                                (satan-tool--pattern-to-jsonschema pattern))))
...
(defun satan-tool--items-jsonschema (items)
...
with `:shape', recurses to build an object schema."
  (cond
   ((symbolp items)
    (list :type (satan-tool--jsonschema-type items)))
    ...
    (let* ((itype   (plist-get items :type))
      ...
      (cond
       ((and (eq itype 'object) shape)
        (satan-tool--args-schema-to-jsonschema shape))
       (t
        (let ((p (list :type (satan-tool--jsonschema-type itype))))
          (when enum    (setq p (plist-put p :enum (vconcat enum))))
          (when pattern (setq p (plist-put p :pattern pattern)))
          p)))))
   (t (error "SATAN: bad :items spec: %S" items))))

(defun satan-tool-json-schema (tool-spec)
  "Return the OpenAI-tools dict for TOOL-SPEC, ready for the manifest.
Description is loaded from `satan-tools-descriptions-dir'."
  (let* ((name (plist-get tool-spec :name))
         (desc (satan-tool--description name))
         (params (satan-tool--args-schema-to-jsonschema
...
is canonical here so every adapter sees the same text."
  (let ((desc (satan-tool--description "satan_final")))
    (list :type "function"
          :function
