;;; satan-memory-canon.el --- canonicalizer (PURE) -*- lexical-binding: t; -*-

;; Pure function `satan-memory-canon-canonicalize':
;;
...
(defconst satan-memory-canon--domain-to-kind
...
(defvar satan-memory-canon--rules nil
  "Ordered list of (RULE-ID . FUNCTION).  Each FUNCTION takes
  (evidence hints ctx) and returns a list of emission plists.")

(defun satan-memory-canon--register-rule (id fn)
  "Register or replace rule ID with handler FN.  Idempotent on reload."
  (setq satan-memory-canon--rules
        (append (cl-remove id satan-memory-canon--rules :key #'car)
                (list (cons id fn)))))

(defmacro satan-memory-canon-defrule (id args docstring &rest body)
...
The dispatcher attaches `:rule-id' and `:grammar-version'."
  ...
  (let ((fn-sym (intern (format "satan-memory-canon--rule/%s" id))))
    `(progn
       (defun ,fn-sym ,args ,docstring ,@body)
       (satan-memory-canon--register-rule ',id #',fn-sym))))

;; ---------------------------------------------------------------------
;; Pure helpers
;; ---------------------------------------------------------------------

(defun satan-memory-canon--slugify (s)
...
(defun satan-memory-canon--app-surface (app-id)
  ...
  (or (cdr (assoc app-id satan-memory-canon--app-to-surface))
      "desktop"))
...
(defun satan-memory-canon--suggest-closed (ns input)
...
or share a prefix with it.  Used to build the rejected suggestions list."
  (let* ((all (satan-memory-grammar-closed-values ns))
...
(defun satan-memory-canon-normalize-hints (hints)
...
capped at `satan-memory-canon--max-topic-count'."
  (let ((norm nil)
        (rejected nil))
    ;; kind
    (let ((k (plist-get hints :kind)))
      (cond
       ...
       ((member k satan-memory-canon--trace-kinds)
        (setq norm (plist-put norm :kind k)))
       (t (push (list :field 'kind :value k
                      :suggestions satan-memory-canon--trace-kinds)
                rejected))))
    ;; phase (alias-resolved, then closed-world)
    (let* ((p (plist-get hints :phase))
           (alias (and p (satan-memory-grammar-alias-target p)))
           (resolved (cond
                      ((null p) nil)
...
(satan-memory-canon-defrule panopticon.current.app (ev _hints _ctx)
...
derived `surface:<value>'."
  (let* ((cw (plist-get ev :current_window))
    ...
    (when app-id
      (let* ((slug (satan-memory-canon--slugify app-id))
             (surface (satan-memory-canon--app-surface app-id)))
        (delq
         nil
         (list
          (and slug
...
(satan-memory-canon-defrule panopticon.surface_transition (ev _hints _ctx)
...
(satan-memory-canon-defrule panopticon.event_transition (_ev _hints _ctx)
...
(satan-memory-canon-defrule panopticon.domain_transition (ev _hints _ctx)
...
transition; concrete domain→domain transitions can land later)."
  (let ((segs (plist-get ev :browser_segments))
    ...
    (cl-loop for (a b) on segs
             when (and a b)
             do (let* ((from (satan-memory-canon--domain-kind
                              (plist-get a :domain)))
                       (to   (satan-memory-canon--domain-kind
                              (plist-get b :domain))))
                  (when (and from to (not (equal from to)))
...
(satan-memory-canon-defrule panopticon.docs_visit (ev _hints _ctx)
...
allowlist within the window."
  (let ((segs (plist-get ev :browser_segments))
    ...
    (while (and segs (null match-idx))
      (when (equal "docs"
                   (satan-memory-canon--domain-kind
...
(satan-memory-canon-defrule panopticon.content (ev _hints _ctx)
  ...
  (let ((idx -1) seen acc)
    (dolist (row (plist-get ev :git_commits) (nreverse acc))
      ...
      (let* ((remote (plist-get row :remote))
             ...
             (slug (and raw (satan-memory-canon--slugify raw))))
...
(satan-memory-canon-defrule ctx.mode (_ev _hints ctx)
  ...
  (let ((mode (plist-get ctx :mode_name)))
    (when (and mode (satan-memory-grammar-valid-value-p 'mode mode))
      (list (satan-memory-canon--emit
             (concat "mode:" mode) 'ctx "/ctx/mode_name")))))

(satan-memory-canon-defrule time.day_week (_ev _hints ctx)
...
(satan-memory-canon-defrule hint.topic (_ev hints _ctx)
...
(satan-memory-canon-defrule hint.focal_app (_ev hints _ctx)
...
(defun satan-memory-canon--origin-rank (origin)
  (or (cdr (assq origin satan-memory-canon--origin-priority)) 99))

(defun satan-memory-canon--merge (emissions grammar-version)
  ...
  (let ((table (make-hash-table :test 'equal)))
    (dolist (e emissions)
      (let* ((h (plist-get e :handle))
        ...
        (when (or (null prev)
                  (< (satan-memory-canon--origin-rank
                      (plist-get e :origin))
                     (satan-memory-canon--origin-rank
...
(defun satan-memory-canon--validate-emission (rule-id e)
...
(defun satan-memory-canon-canonicalize (evidence normalized-hints ctx)
...
`satan-memory-canon-normalize-hints'.  Callers using
`satan-memory-canon-canonicalize-from-raw' get rejections merged."
  (let* ((gv (or (plist-get ctx :current_grammar_version)
                 satan-memory-grammar-current-version))
         (all-emissions
          (cl-loop for (rule-id . fn) in satan-memory-canon--rules
                   for raw = (funcall fn evidence normalized-hints ctx)
                   append (mapcar
                           (lambda (e)
                             (satan-memory-canon--validate-emission
                              rule-id
                              (plist-put (copy-sequence e) :rule-id rule-id)))
                           raw)))
         (merged (satan-memory-canon--merge all-emissions gv)))
    (list :handles (car merged)
          :handle_sources (cadr merged)
          :rejected nil)))

(defun satan-memory-canon-canonicalize-from-raw (evidence raw-hints ctx)
...
running the normalize step a second time."
  (let* ((nh (satan-memory-canon-normalize-hints raw-hints))
         ...
         (canon (satan-memory-canon-canonicalize
...
(provide 'satan-memory-canon)
;;; satan-memory-canon.el ends here
