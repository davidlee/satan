;;; satan-resonance-test.el --- Phase 2 resonance ert -*- lexical-binding: t; -*-
...
(defun satan-resonance-test--percept (handles sources)
...
(defun satan-resonance-test--src (rule-id handle)
  "Source row matching `satan-percept--sources-rows' output for HANDLE."
  (list :handle handle :rule_id rule-id))

(defun satan-resonance-test--ok-stub (matches)
  "Return a `:store-resonate' stub returning (ok . MATCHES) regardless of args."
  (lambda (&rest _) (cons 'ok matches)))

(defun satan-resonance-test--err-stub (msg)
...
(ert-deftest satan-resonance/gate-skip-empty-cue ()
  ...
  (let* ((called 0)
         (percept (satan-resonance-test--percept nil nil))
         (stub (lambda (&rest _) (cl-incf called) (cons 'ok nil)))
         (result (satan-resonance-derive
                  percept (list :store-resonate stub))))
    (should (eq (plist-get result :status) 'gate-skip))
...
(ert-deftest satan-resonance/gate-admits-when-panopticon-observed ()
...
(ert-deftest satan-resonance/limit-forwarded-to-store ()
  ...
  (let* ((passed nil)
    ...
    (satan-resonance-derive percept (list :store-resonate stub))
    (should (= (plist-get passed :limit) 3))
    (setq passed nil)
    (satan-resonance-derive
...
(ert-deftest satan-resonance/zero-matches-yields-no-match-status ()
...
(distinct from gate-skip); block omits."
  (let* ((percept (satan-resonance-test--percept
         ...
         (result (satan-resonance-derive
                  ...
                  (list :store-resonate
                        (satan-resonance-test--ok-stub nil))))
         (block (satan-resonance-render-block
                 '(("resonance_block_header" . "# Resonance"))
                 result)))
    (should (eq (plist-get result :status) 'no-match))
    (should (null block))))

(ert-deftest satan-resonance/psql-down-yields-memory-unreachable ()
...
gate-skip via `:status'."
  (let* ((percept (satan-resonance-test--percept
         ...
         (result (satan-resonance-derive
                  percept
                  (list :store-resonate
                        (satan-resonance-test--err-stub "psql exit 1"))))
         (block (satan-resonance-render-block
                 '(("resonance_block_header" . "# Resonance"))
                 result)))
...
(ert-deftest satan-resonance/render-block-shape ()
         ...
         (result (list :status 'ok
                       ...
                       '((:trace_id "20260518T120000-aaa"
                         ...
                         (:trace_id "20260515T080000-bbb"
                          :score 6.5
                          :matched_handles ("app:firefox" "mode:motd")))))
         (lines (satan-resonance-render-block framing result)))
...
(ert-deftest satan-resonance/render-block-includes-payload-line ()
...
(ert-deftest satan-resonance/render-block-omits-empty-payload-line ()
         ...
         (result (list :status 'ok
                       ...
                       (list (list :trace_id "t-nil" :score 1.0
                             ...
                             (list :trace_id "t-empty" :score 2.0
                                   :matched_handles '("app:firefox")
                                   :payload "")))))
    (should (equal (satan-resonance-render-block framing result)
                   (list "# Resonance"
                         "- t-nil  score 1.0"
                         "    matched: app:firefox"
                         "- t-empty  score 2.0"
...
(ert-deftest satan-resonance/render-block-truncates-long-payload ()
...
(ert-deftest satan-resonance/render-block-without-framing-key-yields-nil ()
  ...
  (let* ((framing '(("percept_block_header" . "# Percept")))
         (result (list :status 'ok
                       :matches '((:trace_id "tid" :score 1.0
                                   :matched_handles ("app:firefox"))))))
    (should (null (satan-resonance-render-block framing result)))))

(ert-deftest satan-resonance/render-block-omits-on-gate-skip ()
...
(ert-deftest satan-resonance/fixture-real-percept-gate-skip-when-no-sensors ()
...
test trips before §S2 silently weakens."
  (satan-resonance-test--with-sensor-fixture (:tmp _t :behaviour beh :run-dir rd)
    (let* ((prepare (satan-resonance-test--prepare
...
(ert-deftest satan-resonance/fixture-real-percept-gate-admits-on-panopticon ()
...
noise floor."
  (satan-resonance-test--with-sensor-fixture (:tmp _t :behaviour beh :run-dir rd)
    (ignore rd)
    (satan-resonance-test--write-sway beh "firefox")
    (let* ((prepare (satan-resonance-test--prepare
      ...
      (let ((result (satan-resonance-derive
                     percept (list :store-resonate stub))))
        (should (eq (plist-get result :status) 'ok))
...
(ert-deftest satan-resonance/fixture-exclude-list-matches-canon-rule-ids ()
  "Lock the gate's exclude list against the canon's actual rule ids.
If a canon rule named in `satan-resonance--excluded-rule-ids' has
...
gate silently weakening because a string compare stopped matching."
  (let ((registered (mapcar (lambda (cell) (symbol-name (car cell)))
                            satan-memory-canon--rules)))
    (dolist (excluded satan-resonance--excluded-rule-ids)
...
(defmacro satan-resonance-test--with-framing (tmp-sym &rest body)
  "Bind a tmp dir to TMP-SYM and seed minimal scaffold + framing + prompt
files under it; rebind `satan-system-scaffold-file' and
`satan-system-framing-file' for BODY's dynamic extent.  Framing
...
(ert-deftest satan-resonance/capsule-renders-resonance-block-from-prepare ()
...
block lands between `# Percept' and `# Today (raw)' per design §S2."
  (satan-resonance-test--with-framing tmp
...
(ert-deftest satan-resonance/capsule-omits-resonance-on-gate-skip ()
...
(ert-deftest satan-resonance/capsule-omits-resonance-when-memory-unreachable ()
...
(ert-deftest satan-resonance/result-json-serializes-via-jsonl-prepare ()
  "Live failure repro: a resonance result with `:status 'ok' (symbol)
must survive `satan-jsonl-prepare' → `json-serialize' so the audit
...
`satan-audit--write-json'."
  ...
  (let* ((result (list :status 'ok
                       ...
                       :matches nil)))
    (should (stringp (json-serialize (satan-jsonl-prepare result)
                                     :null-object :null
                                     :false-object :false)))))

(provide 'satan-resonance-test)
;;; satan-resonance-test.el ends here
