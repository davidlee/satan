...
(ert-deftest satan-tank/truncate-shorter-passes-through ()
  (should (equal (satan-tank--truncate "abc" 10) "abc")))

(ert-deftest satan-tank/truncate-longer-appends-ellipsis ()
  (should (equal (satan-tank--truncate "abcdef" 4) "abc…")))

(ert-deftest satan-tank/short-ts-extracts-clock ()
  (should (equal (satan-tank--short-ts "2026-05-20T08:28:38.199+10:00")
                 "08:28:38")))

(ert-deftest satan-tank/short-run-pulls-mode-slug ()
  (should (equal (satan-tank--short-run
...
(ert-deftest satan-tank/event-summary-tool-call ()
  (should (equal
           (satan-tank--event-summary
            '(:event "tool-call"
              :payload (:name "memory_resonate"
                        :arguments (:limit 5))))
           "memory_resonate(limit=5)")))

(ert-deftest satan-tank/event-summary-tool-result-error ()
  (should (equal
           (satan-tank--event-summary
            '(:event "tool-result"
...
(ert-deftest satan-tank/event-summary-tool-result-ok ()
  (should (equal
           (satan-tank--event-summary
            '(:event "tool-result"
...
(ert-deftest satan-tank/event-summary-timeout ()
  (should (equal
           (satan-tank--event-summary
...
(ert-deftest satan-tank/render-evidence-nil ()
  (let ((out (satan-tank--render-evidence nil)))
    (should (string-match-p "EVIDENCE WINDOW" out))
    (should (string-match-p "(unavailable)" out))))

(ert-deftest satan-tank/render-evidence-populated ()
  (let* ((state
...
(ert-deftest satan-tank/render-attributes-populated ()
  (let* ((snapshot '(("curiosity" . 0.30) ("shame" . 0.50)
                     ("friction" . 0.60)))
         (out (satan-tank--render-attributes snapshot)))
...
(ert-deftest satan-tank/render-attributes-disabled ()
  (let ((out (satan-tank--render-attributes 'disabled)))
    (should (string-match-p "ATTRIBUTES" out))
    (should (string-match-p "(disabled)" out))))

(ert-deftest satan-tank/render-attributes-nil ()
  (let ((out (satan-tank--render-attributes nil)))
    (should (string-match-p "ATTRIBUTES" out))
    (should (string-match-p "(unavailable)" out))))

(ert-deftest satan-tank/gather-attributes-disabled ()
  (let ((satan-attribute-updates-enabled nil))
    (should (eq 'disabled (satan-tank--gather-attributes)))))

(ert-deftest satan-tank/render-traces-empty ()
  (let ((out (satan-tank--render-traces nil)))
    (should (string-match-p "RECENT TRACES (last 0)" out))
    (should (string-match-p "(no traces)" out))))

(ert-deftest satan-tank/render-traces-formats-row ()
  (let* ((rows '((:trace_id "20260520T080000-abc001"
         ...
         (out (satan-tank--render-traces rows)))
  ...
  (let* ((tmp (make-temp-file "satan-tank-runs-" t))
    ...
    (unwind-protect
        (progn
          ...
          (let* ((satan-runs-dir tmp)
                 (events (satan-tank--read-run-events run-id)))
...
(ert-deftest satan-tank/recent-runs-newest-first ()
  (let ((tmp (make-temp-file "satan-tank-runs-" t)))
    (unwind-protect
        (progn
...
(ert-deftest satan-tank/last-run-state-aggregates ()
  ...
  (let* ((events
         ...
         (state (satan-tank--last-run-state
...
(ert-deftest satan-tank/last-run-status-timeout ()
  (let* ((events
          '((:ts "2026-05-20T08:28:08+10:00" :dir "in"
             :event "ready" :payload (:type "ready"))
            (:ts "2026-05-20T08:28:38+10:00" :dir "broker"
             :event "timeout" :payload (:after-seconds 30)))))
    (should (eq (satan-tank--last-run-status events) 'timeout))))

(ert-deftest satan-tank/last-run-status-error ()
  (let* ((events
    ...
    (should (eq (satan-tank--last-run-status events) 'error))))

(ert-deftest satan-tank/render-last-run-nil ()
  (let ((out (satan-tank--render-last-run nil)))
    (should (string-match-p "LAST RUN" out))
    (should (string-match-p "(no runs yet)" out))))

(ert-deftest satan-tank/render-last-run-formats ()
  (let* ((state (list :run_id "20260520T092127-tick-pulse-1e8e70"
                      ...
                      :final_actions 0))
         (out (satan-tank--render-last-run state)))
    (should (string-match-p "LAST RUN" out))
    (should (string-match-p "tick-pulse" out))
