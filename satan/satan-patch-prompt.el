;;; satan-patch-prompt.el --- harness directive assembler -*- lexical-binding: t; -*-
...
;; `satan-patch-adapter' module commentary for the contract.
...
(defcustom satan-patch-prompt-timeout-seconds 1800
  "Default wall-clock cap for an adapter run."
  :type 'integer :group 'satan-patch)

(defcustom satan-patch-prompt-max-output-bytes (* 8 1024 1024)
...
(defun satan-patch-prompt--render-source (source)
...
(defun satan-patch-prompt-build-directive (job)
  "Build the directive string passed to the adapter.

JOB is a row plist as returned by `satan-patch-store-get'.
...
context bundle, and the commit-message footer the agent must use."
  (let* ((job-id    (plist-get job :id))
         ...
         (rendered-context (satan-patch-prompt--render-context context))
         (source-line (satan-patch-prompt--render-source source)))
    (mapconcat
     #'identity
     (delq nil
           (list
            ...
            "## Allowed paths"
            ""
            (satan-patch-prompt--render-allowlist allowed)
            ""
            "## Checks"
            ""
            (satan-patch-prompt--render-checks checks)
...
(defun satan-patch-prompt-log-path (job-id)
  "Return the canonical adapter-log path for JOB-ID."
  (expand-file-name (concat job-id ".jsonl")
                    satan-patch-prompt-log-root))

(cl-defun satan-patch-prompt-build
    (job &key
         (system-prompt-file satan-patch-prompt-system-file)
         (timeout-seconds satan-patch-prompt-timeout-seconds)
         (max-output-bytes satan-patch-prompt-max-output-bytes)
         provider model)
...
provider/model.  Ensures the log directory exists."
  (let* ((job-id (plist-get job :id))
         (log-path (satan-patch-prompt-log-path job-id)))
    (make-directory (file-name-directory log-path) t)
    (list :system-prompt-file system-prompt-file
          :directive (satan-patch-prompt-build-directive job)
...
(provide 'satan-patch-prompt)
;;; satan-patch-prompt.el ends here
