;;; satan-audit.el --- SATAN run audit log + verifier -*- lexical-binding: t; -*-
...
;;
;; `satan-audit-verify-run' answers the six predicates that together prove
...
(cl-defstruct satan-audit-handle
  dir
  transcript-path
  last-ts
...
finishes the open by invoking `satan-audit-attach-bundle' once
the bundle is built."
  (unless (file-directory-p dir) (make-directory dir t))
  (satan-audit--write-json (expand-file-name "manifest.json" dir) manifest)
  (when bundle
    (satan-audit--write-json (expand-file-name "bundle.json" dir) bundle))
  (let ((tp (expand-file-name "transcript.jsonl" dir)))
    (with-temp-file tp (insert ""))
    (make-satan-audit-handle :dir dir :transcript-path tp :last-ts nil
                                :run-ctx run-ctx)))

(defun satan-audit-attach-bundle (handle bundle)
  "Write BUNDLE plist as `bundle.json' under HANDLE's run directory.
Used by the broker when `satan-audit-open' was called with a nil
bundle to allow pre-bundle observer emits (T7 PR 5)."
  (satan-audit--write-json
   (expand-file-name "bundle.json" (satan-audit-handle-dir handle))
   bundle))

(defun satan-audit-reopen (dir)
  "Return an append-only audit handle for an EXISTING run DIR.
Unlike `satan-audit-open', this does NOT truncate the transcript
...
manual marks must attach to a real prior run, not mint a new one."
  (unless (file-directory-p dir)
    (user-error "satan-audit-reopen: not a directory: %s" dir))
  (let ((tp (expand-file-name "transcript.jsonl" dir)))
    (unless (file-readable-p tp)
      (user-error "satan-audit-reopen: no transcript.jsonl in %s" dir))
    (make-satan-audit-handle :dir dir :transcript-path tp :last-ts nil
                                :run-ctx nil)))

(defun satan-audit-record (handle dir event payload)
  "Append a transcript record.
DIR ∈ in|out|broker.  EVENT is a symbol.  PAYLOAD is a plist/list/string."
  (let* ((ts (satan-audit--iso-now))
         (rec (list :ts ts
                    :dir (symbol-name dir)
                    :event (symbol-name event)
                    :payload (or payload :null))))
    (setf (satan-audit-handle-last-ts handle) ts)
    (satan-audit--append-line
     (satan-audit-handle-transcript-path handle)
     (json-serialize (satan-jsonl-prepare rec)
                     :null-object :null :false-object :false))))

(defun satan-audit-close (handle final actions status)
  "Finalize the run.
...
STATUS is a symbol."
  (let ((dir (satan-audit-handle-dir handle))
...
(defun satan-audit--list-of-plists-p (val)
...
(defun satan-audit-validate-actions (obj)
...
`satan-audit-p/actions-partition-final', which reads from disk
and knows about `final.json'."
  (cond
   ((not (listp obj)) "actions must be plist")
   ((not (satan-audit--list-of-plists-p (plist-get obj :applied)))
    "applied must be array")
   ((not (satan-audit--list-of-plists-p (plist-get obj :staged)))
    "staged must be array")
   ((not (satan-audit--list-of-plists-p (plist-get obj :rejected)))
    "rejected must be array")
   ((not (satan-audit--list-of-plists-p (plist-get obj :failed)))
    "failed must be array")
   ((plist-member obj :pre_spawn)
    (satan-audit--validate-pre-spawn (plist-get obj :pre_spawn)))))

(defun satan-audit--validate-pre-spawn (val)
...
(defun satan-audit--iv-key-name (key)
  (substring (symbol-name key) 1))

(defun satan-audit--iv-require-string (payload key)
  "Require KEY in PAYLOAD to be a non-empty string."
  (cond
   ((not (plist-member payload key))
    (format "missing required field: %s" (satan-audit--iv-key-name key)))
   ((not (stringp (plist-get payload key)))
    (format "field %s must be string" (satan-audit--iv-key-name key)))
   ((string-empty-p (plist-get payload key))
    (format "field %s must be non-empty" (satan-audit--iv-key-name key)))))

(defun satan-audit--iv-require-enum (payload key allowed)
  "Require KEY in PAYLOAD to be a string drawn from ALLOWED."
  (or (satan-audit--iv-require-string payload key)
      (unless (member (plist-get payload key) allowed)
        (format "field %s must be one of %S, got %S"
                (satan-audit--iv-key-name key)
                allowed
                (plist-get payload key)))))

(defun satan-audit--iv-require-integer (payload key)
  "Require KEY in PAYLOAD to be a non-negative integer."
  (cond
   ((not (plist-member payload key))
    (format "missing required field: %s" (satan-audit--iv-key-name key)))
   ((not (integerp (plist-get payload key)))
    (format "field %s must be integer" (satan-audit--iv-key-name key)))
   ((< (plist-get payload key) 0)
    (format "field %s must be non-negative" (satan-audit--iv-key-name key)))))

(defun satan-audit--iv-require-array (payload key)
  "Require KEY in PAYLOAD to be a JSON array.
Accepts lists (`nil' counts as empty array) and vectors (the round-trip
representation produced by `satan-attribute-listener--claim-row'
parsing with `:array-type 'vector')."
  (cond
   ...
   ((let ((v (plist-get payload key)))
    ...
    (format "field %s must be array" (satan-audit--iv-key-name key)))))

(defun satan-audit--iv-require-object (payload key)
...
(defun satan-audit--iv-require-string-or-null (payload key)
...
(defun satan-audit--validate-intervention-created (payload)
  "Validate an `intervention.created' payload.  Return error string or nil."
  (or (satan-audit--iv-require-string  payload :intervention_id)
      (satan-audit--iv-require-string  payload :run_id)
      (satan-audit--iv-require-string  payload :ts)
      (satan-audit--iv-require-string  payload :mode)
      (satan-audit--iv-require-enum    payload :kind
                                          satan-audit-intervention-kinds)
      (satan-audit--iv-require-string  payload :target_surface)
      (satan-audit--iv-require-string  payload :message)
      (satan-audit--iv-require-string-or-null payload :related_motive_id)
      (satan-audit--iv-require-array   payload :cue_handles)
      (satan-audit--iv-require-array   payload :percept_handles)
      (satan-audit--iv-require-string  payload :expected_outcome)
      (satan-audit--iv-require-integer payload :outcome_window_minutes)
      (satan-audit--iv-require-enum    payload :severity
                                          satan-audit-intervention-severities)))

(defun satan-audit--validate-intervention-outcome (payload revision-p created-ids)
...
nil or an error string."
  (or (satan-audit--iv-require-string payload :intervention_id)
      ...
      (satan-audit--iv-require-enum payload :classification
                                       satan-audit-intervention-classifications)
      (satan-audit--iv-require-enum payload :confidence
                                       satan-audit-intervention-confidences)
      (satan-audit--iv-require-object payload :evidence)
      (satan-audit--iv-require-enum payload :maturity
                                       satan-audit-intervention-maturities)
      (satan-audit--iv-require-string payload :next_revisit_at)
      (satan-audit--iv-require-enum payload :source
                                       satan-audit-intervention-sources)
      (satan-audit--iv-require-string payload :classified_at)
...
inserts after a successful `created' record."
  (cond
   ...
   ((not (member event satan-audit-intervention-events))
   ...
   (t
    (pcase event
      ("intervention.created"
       (satan-audit--validate-intervention-created payload))
      ("intervention.outcome_classified"
       (satan-audit--validate-intervention-outcome payload nil created-ids))
      ("intervention.outcome_revised"
       (satan-audit--validate-intervention-outcome payload t created-ids))))))

(defun satan-audit-validate-intervention-stream (events)
...
(defconst satan-audit-attribute-events
  '("attribute.delta_applied")
  "Closed set of attribute audit-event names.")

(defconst satan-audit-attribute-names
...
(defun satan-audit--iv-require-number-in-range (payload key lo hi)
  "Require KEY in PAYLOAD to be a finite number in `[LO, HI]'."
  (cond
   ((not (plist-member payload key))
    (format "missing required field: %s" (satan-audit--iv-key-name key)))
   ((not (numberp (plist-get payload key)))
    (format "field %s must be number" (satan-audit--iv-key-name key)))
   ((let ((v (plist-get payload key)))
      (or (< v lo) (> v hi)))
...
(defun satan-audit--iv-require-bool (payload key)
...
(defun satan-audit--attribute-reasons-for-source (source)
  "Return the closed reason enum for an IMPLEMENTED SOURCE, else nil."
  (cond
   ((equal source "outcome") satan-audit-attribute-outcome-reasons)
   ((equal source "hippocampus") satan-audit-attribute-hippocampus-reasons)
   ((equal source "sensor") satan-audit-attribute-sensor-reasons)
   (t nil)))

(defun satan-audit--validate-attribute-source-and-reason (payload)
          ...
          (seq-doseq (c caps)
            (cond
             ((not (stringp c))
              ...
              (throw 'done nil))
             ((not (member c satan-audit-attribute-caps))
              (setq err (format "caps_applied[%d]=%S not in closed set %S"
                                idx c satan-audit-attribute-caps))
              (throw 'done nil)))
            (cl-incf idx)))
        err)))

(defun satan-audit--validate-attribute-outcome-evidence (payload)
...
cue-dimension fields (design-contract §5.1)."
  (let ((ev (plist-get payload :evidence)))
    (or (satan-audit--iv-require-string ev :intervention_id)
        (satan-audit--iv-require-enum ev :classification
                                         satan-audit-intervention-classifications)
        (satan-audit--iv-require-enum ev :confidence
                                         satan-audit-intervention-confidences))))

(defun satan-audit--validate-attribute-hippocampus-evidence (payload)
  "For `source=hippocampus', enforce that `:evidence' carries the required
fields (design-contract §6H.5)."
  (let ((ev (plist-get payload :evidence)))
    (or (satan-audit--iv-require-string ev :tool_name)
        (satan-audit--iv-require-string ev :filename))))

(defun satan-audit--validate-attribute-sensor-evidence (payload)
  "For `source=sensor', enforce that `:evidence' carries the required
fields (design-contract §6S.5)."
  (let ((ev (plist-get payload :evidence)))
    (or (satan-audit--iv-require-string ev :sensor_type)
        (unless (numberp (plist-get ev :metric_value))
          "evidence.metric_value must be a number")
        (satan-audit--iv-require-string ev :metric_unit))))

(defun satan-audit--validate-attribute-delta-applied (payload)
  "Validate an `attribute.delta_applied' payload.  Return error string or nil."
  (or (satan-audit--iv-require-string payload :id)
      (satan-audit--iv-require-enum payload :scope
                                       satan-audit-attribute-scopes)
      (satan-audit--iv-require-enum payload :name
                                       satan-audit-attribute-names)
      (satan-audit--iv-require-number-in-range payload :old 0 1)
      (satan-audit--iv-require-number-in-range payload :new 0 1)
      (satan-audit--iv-require-number-in-range payload :delta -1 1)
      ...
      (satan-audit--validate-attribute-source-and-reason payload)
      (satan-audit--iv-require-object payload :evidence)
      (let ((src (plist-get payload :source)))
        (cond
         ((equal src "outcome")
          (satan-audit--validate-attribute-outcome-evidence payload))
         ((equal src "hippocampus")
          (satan-audit--validate-attribute-hippocampus-evidence payload))
         ((equal src "sensor")
          (satan-audit--validate-attribute-sensor-evidence payload))))
      (satan-audit--validate-attribute-caps payload)
      (satan-audit--iv-require-bool payload :disabled)))

(defun satan-audit-validate-attribute-event (event payload)
  "Validate an attribute audit-log EVENT with PAYLOAD.
EVENT is one of `satan-audit-attribute-events'.  Returns nil on
success or an error string on failure."
  (cond
   ((not (stringp event)) "event must be string")
   ((not (member event satan-audit-attribute-events))
   ...
   (t
    (pcase event
      ("attribute.delta_applied"
       (satan-audit--validate-attribute-delta-applied payload))))))

;; ---------- Verifier ----------

(defun satan-audit--read-json (path)
...
(defun satan-audit-p/calls-match-results (dir)
...
(defun satan-audit-p/pre-spawn-shape (dir)
...
the {applied,staged,rejected,failed} partition invariant."
  (let* ((actions (ignore-errors
    ...
    (cond
     ...
     (t (null (satan-audit--validate-pre-spawn
               (plist-get actions :pre_spawn)))))))

(defun satan-audit-p/status-terminal (dir)
...
(defun satan-audit-verify-run (dir)
  "Return t if all audit predicates pass for DIR.
Otherwise return an alist of (PREDICATE-SYMBOL . nil) pairs."
  (let ((checks
         (list
          (cons 'has-manifest        (satan-audit-p/has-manifest dir))
          (cons 'has-bundle          (satan-audit-p/has-bundle dir))
          (cons 'transcript-monotonic (satan-audit-p/transcript-monotonic dir))
          (cons 'calls-match-results (satan-audit-p/calls-match-results dir))
          (cons 'actions-partition-final (satan-audit-p/actions-partition-final dir))
          (cons 'pre-spawn-shape     (satan-audit-p/pre-spawn-shape dir))
          (cons 'status-terminal     (satan-audit-p/status-terminal dir)))))
    (let ((failed (cl-remove-if #'cdr checks)))
      (if (null failed) t failed))))

(provide 'satan-audit)
;;; satan-audit.el ends here
