...
(cl-defun dl-satan-sensor-curiosity-probe-read (&key run-id ts)
  ...
  (condition-case err
      (when (and run-id (bound-and-true-p dl-satan-attribute-updates-enabled))
        (let* ((last (dl-satan-sensor-curiosity--last-inspected))
               ...
               (count-high (dl-satan-sensor-curiosity--count-uninspected since))
...
(defun dl-satan-sensor-curiosity-probe-commit (snapshot)
  "Commit SNAPSHOT from `dl-satan-sensor-curiosity-probe-read' (consume-side).
...
the broker wall-clock).  Returns non-nil iff a signal was emitted."
  (condition-case err
      (when (plist-get snapshot :emit)
        ...
        (dl-satan-sensor-curiosity-mark-inspected (plist-get snapshot :high-water))
        t)
    (error
...
signal was emitted."
  (dl-satan-sensor-curiosity-probe-commit
   (dl-satan-sensor-curiosity-probe-read :run-id run-id :ts ts)))
