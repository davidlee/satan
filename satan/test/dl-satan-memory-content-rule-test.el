;;; dl-satan-memory-content-rule-test.el --- ert for panopticon.content rule -*- lexical-binding: t; -*-
...
(ert-deftest dl-satan-memory-content/probe-returns-ok-with-metadata ()
  ...
  (dl-satan-tools-content-test--with-store
    (let ((article1 (dl-satan-tools-content-test--article-plist
      ...
      (let* ((result (dl-satan-memory-evidence--content-probe 10))
             (status (car result))
             (captures (cdr result)))
...
(ert-deftest dl-satan-memory-content/probe-empty-store-returns-missing ()
...
(ert-deftest dl-satan-memory-content/probe-respects-limit ()
  ...
  (dl-satan-tools-content-test--with-store
    (let ((articles
      ...
      (dl-satan-tools-content-test--write-article-jsonl articles)
      (let* ((result (dl-satan-memory-evidence--content-probe 3))
             (captures (cdr result)))
        (should (equal "ok" (car result)))
        (should (= 3 (length captures)))))))

(ert-deftest dl-satan-memory-content/probe-malformed-line-skipped ()
...
(defun dl-satan-memory-content-test--apply-rule (evidence &optional hints ctx)
  "Apply the panopticon.content rule to EVIDENCE, return emission handles."
  (let ((emissions (dl-satan-memory-canon--rule/panopticon.content
                    evidence (or hints '()) (or ctx '()))))
    (mapcar (lambda (e) (plist-get e :handle)) emissions)))

(ert-deftest dl-satan-memory-content/rule-emits-content-domain-handles ()
  ...
  (let ((evidence (list :content_recent
    ...
    (let ((handles (dl-satan-memory-content-test--apply-rule evidence)))
...
(ert-deftest dl-satan-memory-content/rule-empty-content-recent ()
  "Empty :content_recent → no emissions."
  (let ((evidence (list :content_recent '())))
    (let ((handles (dl-satan-memory-content-test--apply-rule evidence)))
      (should (null handles)))))

(ert-deftest dl-satan-memory-content/rule-no-content-recent ()
...
(ert-deftest dl-satan-memory-content/panopticon-content-admits-resonance ()
  ...
  (let ((sources (list (list :rule_id "panopticon.content"
    ...
    (should (dl-satan-resonance--admittable-p sources))))

(ert-deftest dl-satan-memory-content/only-excluded-rules-fail-gate ()
  ...
  (let ((sources (list (list :rule_id "ctx.mode" :handle "mode:tick-pulse")
    ...
    (should-not (dl-satan-resonance--admittable-p sources))))

(provide 'dl-satan-memory-content-rule-test)
;;; dl-satan-memory-content-rule-test.el ends here
