;;; satan-audit-test.el --- ert tests for satan-audit -*- lexical-binding: t; -*-

;; Run from CLI:
;;   emacs --batch \
;;     -L ~/.emacs.d/core -L ~/.emacs.d/satan -L ~/.emacs.d/satan/test \
;;     -l satan-audit-test.el -f ert-run-tests-batch-and-exit

(require 'ert)
(require 'cl-lib)
(require 'json)
(require 'satan-audit)
(require 'satan-protocol)             ; fixtures shared with protocol-test

(defun satan-audit-test--write-run (dir final actions status &optional transcript)
...
triples passed verbatim to `satan-audit-record'."
  (make-directory dir t)
  (let ((audit (satan-audit-open dir
                                    '(:run_id "r" :mode (:name "test"))
                                    '(:bundle t))))
    (dolist (rec (or transcript '()))
      (satan-audit-record audit (nth 0 rec) (nth 1 rec) (nth 2 rec)))
    (satan-audit-close audit final actions status)))

(ert-deftest satan-audit/verifier-ok ()
  (let ((dir (make-temp-file "satan-run-" t)))
    (unwind-protect
        (progn
          (satan-audit-test--write-run
           dir
           '(:summary "s" :actions ())
...
(ert-deftest satan-audit/pre-spawn-key-written-when-present ()
  "`satan-audit-close' writes `:pre_spawn' into actions.json when the
...
(ert-deftest satan-audit/pre-spawn-omitted-when-absent ()
...
(ert-deftest satan-audit/verifier-accepts-pre-spawn-run ()
...
(ert-deftest satan-audit/verifier-rejects-malformed-pre-spawn ()
    ...
    (unwind-protect
        (progn
          (satan-audit-test--write-run
           ...
           'done)
          (let ((res (satan-audit-verify-run dir)))
            (should (consp res))
            (should (assq 'pre-spawn-shape res))))
      (delete-directory dir t))))

(ert-deftest satan-audit/verifier-accepts-unknown-pre-spawn-kind ()
...
(ert-deftest satan-audit/validate-actions-pure ()
  "`satan-audit-validate-actions' is a pure (in-memory) validator over
the actions.json shape — usable from fixtures without touching disk."
  (should (null (satan-audit-validate-actions
                 '(:applied () :staged () :rejected () :failed ()))))
  (should (null (satan-audit-validate-actions
                 (list :applied () :staged () :rejected () :failed ()
                       :pre_spawn (list (list :kind "sensor_alert"
                                              :cause "x" :message "y"))))))
  (should (stringp (satan-audit-validate-actions
                    (list :applied () :staged () :rejected () :failed ()
                          :pre_spawn (list (list :cause "no_kind")))))))

;; ---------- actions fixtures (cross-cutter: fixtures shipped with
;; satan-protocol; assertion subject is satan-audit-validate-actions) ----------

(ert-deftest satan-audit/fixtures-actions-valid-pass ()
    ...
    (dolist (entry (satan-protocol-fixtures))
      (when (and (string= (plist-get entry :kind) "valid")
        ...
        (let* ((msg (plist-get entry :message))
               (err (satan-audit-validate-actions msg))
               (name (plist-get entry :name)))
          (should (null err))
          (ignore name))))
    (should (> seen 0))))

(ert-deftest satan-audit/fixtures-actions-invalid-fail ()
...
(ert-deftest satan-audit/verifier-detects-orphan-call ()
  (let ((dir (make-temp-file "satan-run-" t)))
    (unwind-protect
        (progn
          (satan-audit-test--write-run
...
(ert-deftest satan-audit/reopen-appends-without-truncating ()
  "`satan-audit-reopen' returns a handle whose `audit-record' appends
        ...
        (progn
          ...
          (let* ((tp (expand-file-name "transcript.jsonl" dir))
                 (before (with-temp-buffer (insert-file-contents tp)
                                           (buffer-string)))
                 (handle (satan-audit-reopen dir)))
            (satan-audit-record handle 'broker 'intervention.outcome_classified
                                   '(:intervention_id "iv1"
...
(ert-deftest satan-audit/reopen-rejects-missing-dir ()
  (let ((dir (make-temp-file "satan-reopen-nope-" t)))
    (delete-directory dir t)
    (should-error (satan-audit-reopen dir) :type 'user-error)))

(ert-deftest satan-audit/reopen-rejects-dir-without-transcript ()
  (let ((dir (make-temp-file "satan-reopen-bare-" t)))
    (unwind-protect
        (should-error (satan-audit-reopen dir) :type 'user-error)
      (delete-directory dir t))))

(provide 'satan-audit-test)
;;; satan-audit-test.el ends here
