;;; satan-percept-test.el --- percept skeleton ert -*- lexical-binding: t; -*-
...
(ert-deftest satan-percept/build-returns-shape ()
  "Build returns the documented plist shape with run_id + time_now
mirrored from PREPARE and the canon's emitted handles."
  (satan-percept-test--with-fixture (:tmp tmp :behaviour beh :run-dir rd)
    (satan-percept-test--write-sway beh "firefox")
...
(ert-deftest satan-percept/build-empty-sources-yields-only-ctx-handles ()
...
(ert-deftest satan-percept/persist-writes-percept-json ()
...
(ert-deftest satan-percept/byte-identical-rerun-on-frozen-inputs ()
  "A3 — two builds over the same frozen sensor fixture + frozen
time_now produce byte-identical `percept.json' encodings.

The percept persist path uses `satan-audit--write-json' which
canonicalizes via `satan-jsonl-prepare' before serializing, so
re-emitted JSON is comparable byte-for-byte."
  (satan-percept-test--with-fixture (:tmp tmp :behaviour beh :run-dir rd)
    ...
    (let* ((prepare (satan-percept-test--prepare
      ...
      (satan-audit--write-json path-one one)
      (satan-audit--write-json path-two two)
...
(ert-deftest satan-percept/render-block-uses-framing-header ()
...
canon's emitted handles (no absence padding)."
  (let* ((framing '(("percept_block_header" . "# Percept")))
         ...
         (lines (satan-percept-render-block framing percept)))
    (should (equal (car lines) "# Percept"))
    (should (equal (cdr lines) '("- app:firefox" "- surface:browser")))))

(ert-deftest satan-percept/render-block-empty-handles-yields-nil ()
  "A6 — empty handle list returns nil so the capsule omits the block
entirely (rather than emitting an empty `# Percept' header)."
  (let* ((framing '(("percept_block_header" . "# Percept")))
         (percept '(:handles ())))
    (should (null (satan-percept-render-block framing percept)))))

(ert-deftest satan-percept/render-block-without-framing-key-yields-nil ()
...
(defconst satan-percept-test--attention-framing
  '(("attention_block_header" . "# Recent attention")))

(ert-deftest satan-percept/attention-block-interleaves-focus-and-browser ()
          ...
          (list :evidence_window
                (list :focus_segments
                      ...
                      (list (list :source "firefox"
                                  ...
                                  :duration_s 65)))))
         (lines (satan-percept-render-attention-block
                 satan-percept-test--attention-framing percept)))
...
(ert-deftest satan-percept/attention-block-drops-browser-app-focus ()
...
(ert-deftest satan-percept/attention-block-without-framing-key-yields-nil ()
  ...
  (let ((percept (list :evidence_window
                       (list :focus_segments
                             (list (list :app_id "Emacs" :start_ts "x"
                                         :duration_s 10))))))
    (should (null (satan-percept-render-attention-block
                   '(("now" . "# Now")) percept)))))

(ert-deftest satan-percept/attention-block-caps-at-limit ()
  "Only the most-recent `satan-percept-attention-limit' segments render."
  (let* ((satan-percept-attention-limit 2)
         (mk (lambda (n)
               (list :app_id "Emacs"
...
(ert-deftest satan-percept/attention-block-drops-subsecond-segments ()
...
(ert-deftest satan-percept/determinism-on-rich-fixture ()
...
random ordering."
  (satan-percept-test--with-fixture (:tmp tmp :behaviour beh :run-dir rd)
    (ignore tmp)
    (satan-percept-test--seed-fixture beh "2026-05-19")
    (let* ((prepare (satan-percept-test--prepare
      ...
      (satan-percept-persist
       (file-name-directory path-a)
       (satan-percept-build prepare '(:name "motd") opts))
      (satan-percept-persist
...
(ert-deftest satan-percept/bundle-and-percept-share-identity ()
  "A2 — writes both bundle.json (via a context-fn round-trip and
`satan-audit--write-json') and percept.json from the same prepare
...
ships."
  (satan-percept-test--with-fixture (:tmp tmp :behaviour beh :run-dir rd)
    (satan-percept-test--write-sway beh "firefox")
    (let* ((satan-system-scaffold-file
      ...
      (let* ((mode (list :name "motd"
             ...
             (prepare (satan-percept-test--prepare
             ...
             (prepare-with-percept
              (plist-put (plist-put prepare :evidence
                         ...
                         :percept percept))
             (bundle (satan-context-motd mode prepare-with-percept))
             (bundle-path (expand-file-name "bundle.json" rd))
             (percept-path (satan-percept-persist rd percept)))
        (satan-audit--write-json bundle-path bundle)
        (let ((bundle-on-disk
               (with-temp-buffer
...
;; ---------------------------------------------------------------------
;; Capsule render through satan-context (1.3)
;; ---------------------------------------------------------------------

(require 'satan-context)

(ert-deftest satan-percept/capsule-renders-percept-block-from-prepare ()
...
(ert-deftest satan-percept/capsule-omits-percept-block-when-no-prepare ()
    ...
    (unwind-protect
        (progn
          ...
          (let* ((spec (list :name "motd"
                             :prompt-file
                             (expand-file-name "prompts/motd.txt" tmp)))
                 (bundle (satan-context-motd spec nil))
                 (prompt (plist-get bundle :prompt)))
            (should-not (string-match-p "^# Percept$" prompt))))
      (delete-directory tmp t))))

(provide 'satan-percept-test)
;;; satan-percept-test.el ends here
