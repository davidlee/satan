;;; satan-memory-migrate.el --- SATAN memory migration runner -*- lexical-binding: t; -*-

;; Forward-only numbered SQL migrations for the `satan_memory' PG database.
;; Files live under `satan-memory-migrate-directory' and match
...
(defun satan-memory-migrate--applied (db)
...
(defun satan-memory-migrate--sql-literal (s)
  "Quote S as a single-quoted SQL literal."
  (concat "'" (replace-regexp-in-string "'" "''" s) "'"))

(defun satan-memory-migrate--apply-one (db row)
...
row in the same transaction."
  (let* ((version  (plist-get row :version))
         ...
         (checksum (satan-memory-migrate--checksum path))
         (script   (concat
                    (format "\\i %s\n" path)
                    (format "INSERT INTO schema_migrations (version, filename, checksum) VALUES (%d, %s, %s);\n"
                            version
                            (satan-memory-migrate--sql-literal filename)
                            (satan-memory-migrate--sql-literal checksum))))
...
(defun satan-memory-migrate-status (&optional db)
...
- missing:  recorded as applied but no on-disk file."
  (let* ((db        (or db satan-memory-migrate-database))
         (files     (satan-memory-migrate--list-files))
         (applied   (satan-memory-migrate--applied db))
    ...
    (dolist (f files)
      (let* ((v (plist-get f :version))
             (cs (satan-memory-migrate--checksum (plist-get f :path)))
...
(defun satan-memory-migrate-apply (&optional db)
...
would skip (must be max(applied)+1, max+2, ...)."
  (let* ((db (or db satan-memory-migrate-database))
         (status (satan-memory-migrate-status db))
    ...
    (let* ((pending (cl-remove-if-not
      ...
      (cl-loop for entry in pending
               ...
               collect (let* ((file (cl-find v (satan-memory-migrate--list-files)
                                             :key (lambda (r) (plist-get r :version)))))
                        (satan-memory-migrate--apply-one db file)
                        v)))))

;;;###autoload
(defun satan-memory-migrate (&optional db)
  ...
  (let ((applied (satan-memory-migrate-apply db)))
    (message "satan_memory: applied %d migration(s) %s"
             (length applied) applied)))

;;;###autoload
(defun satan-show-migrate-status (&optional db)
  "Print migration status for DB."
  (interactive
   (list (if current-prefix-arg
             (read-string "Database: " satan-memory-migrate-database)
           satan-memory-migrate-database)))
  (let ((status (satan-memory-migrate-status db)))
...
;;
;; `satan-memory-renormalize' replays the canonicalizer over every
...
(defun satan-memory-renormalize--require ()
  (require 'satan-memory-grammar)
  (require 'satan-memory-canon))

(defun satan-memory-renormalize--mode-from-source (source)
...
(defun satan-memory-renormalize--parse-metadata (s)
...
(defun satan-memory-renormalize--fetch-traces (db)
...
(defun satan-memory-renormalize--build-ctx (row version)
  "Build the canon ctx plist for ROW under VERSION."
  (list :current_grammar_version version
        :mode_name (satan-memory-renormalize--mode-from-source
                    (plist-get row :source))
        :time_now (plist-get row :observed_end_at)
        :run_id nil
        :run_started_at nil))

(defun satan-memory-renormalize--source-jsonb (source grammar-version)
...
(defun satan-memory-renormalize--apply-sql (row version canon)
  ...
  (let* ((tid (plist-get row :trace_id))
         ...
         (tid-lit (satan-memory-migrate--sql-literal tid))
         (rows
          (mapconcat
           (lambda (h)
             (let* ((src (cdr (assoc h sources)))
                    (json (satan-memory-renormalize--source-jsonb
                           src version)))
               (format "(%s,%d::smallint,%s,%s::jsonb,TRUE)"
                       tid-lit
                       version
                       (satan-memory-migrate--sql-literal h)
                       (satan-memory-migrate--sql-literal json))))
           handles
           ",")))
    (concat
     "BEGIN;\n"
...
(defun satan-memory-renormalize--one (db version row)
...
its own condition-case)."
  (let* ((metadata (satan-memory-renormalize--parse-metadata
                    (plist-get row :metadata_json)))
         (evidence (plist-get metadata :evidence))
         (raw-hints (plist-get metadata :hints))
         (ctx (satan-memory-renormalize--build-ctx row version))
    ...
    (if (equal new-handles current)
        'skipped
      (let* ((sql (satan-memory-renormalize--apply-sql row version canon))
             ;; Per-trace renormalize write — part of the migration path,
             ;; never killed.  :timeout-secs nil = unbounded.
             (result (satan-db-psql db satan-memory-migrate-host satan-memory-migrate-psql-program (list "-f" "-") sql
                                       :timeout-secs nil)))
        (pcase result
          (`(ok . ,_) 'updated)
          (`(error . ,msg)
...
(defun satan-memory-renormalize (&optional db version)
  "Replay canonicalization for every trace under VERSION.
DB defaults to `satan-memory-migrate-database'; VERSION to the
...
no-op pass touches zero rows."
  (satan-memory-renormalize--require)
  (let* ((db (or db satan-memory-migrate-database))
         (version (or version satan-memory-grammar-current-version))
         (rows (satan-memory-renormalize--fetch-traces db))
         (updated 0)
         (skipped 0)
         failed)
    (dolist (row rows)
      (condition-case err
          (pcase (satan-memory-renormalize--one db version row)
...
(defun satan-memory-renormalize-status (&optional db)
  "Read-only summary of trace_handles activity per grammar_version.
DB defaults to `satan-memory-migrate-database'.  Returns plist
...
grammar_version is below the current elisp grammar version."
  (satan-memory-renormalize--require)
  (let* ((db (or db satan-memory-migrate-database))
         (current satan-memory-grammar-current-version)
         (sql
          (concat
           "WITH per_trace AS ("
...
;;;###autoload
(defun satan-renormalize-memory (&optional db)
...
With prefix arg, prompt for DB."
  (interactive
   (list (if current-prefix-arg
             (read-string "Database: " satan-memory-migrate-database)
           satan-memory-migrate-database)))
  (let* ((before (satan-memory-renormalize-status db))
         (result (satan-memory-renormalize db))
         (after  (satan-memory-renormalize-status db)))
    (message
...
(provide 'satan-memory-migrate)
;;; satan-memory-migrate.el ends here
