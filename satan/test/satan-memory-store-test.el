;;; satan-memory-store-test.el --- store backend ert -*- lexical-binding: t; -*-
...
(defconst satan-memory-store-test--db "satan_memory_test")

(defun satan-memory-store-test--reachable-p ()
  (pcase (let ((satan-memory-migrate-database
                satan-memory-store-test--db))
           (satan-db-psql
            satan-memory-store-test--db satan-memory-migrate-host satan-memory-migrate-psql-program
            (list "-A" "-t" "-c" "SELECT 1")))
    (`(ok . ,_) t)
    (_ nil)))

(defun satan-memory-store-test--reset-and-migrate ()
  (let ((satan-memory-migrate-database satan-memory-store-test--db))
    (satan-db-psql
     satan-memory-store-test--db satan-memory-migrate-host satan-memory-migrate-psql-program
     (list "-c"
           (concat
...
(defmacro satan-memory-store-test--with-db (&rest body)
  (declare (indent 0))
  `(progn
     (skip-unless (satan-memory-store-test--reachable-p))
     (satan-memory-store-test--reset-and-migrate)
...
(ert-deftest satan-memory-store/trace-id-format ()
  (let* ((id (satan-memory-store-trace-id-new
              "2026-05-19T10:00:00+10:00"
              (lambda () "abc123"))))
    (should (equal id "20260519T100000-abc123"))))

(ert-deftest satan-memory-store/trace-id-random-suffix-length ()
  (let* ((id (satan-memory-store-trace-id-new
              "2026-05-19T10:00:00+10:00")))
    (should (string-match-p "\\`[0-9]\\{8\\}T[0-9]\\{6\\}-[a-z0-9]\\{6\\}\\'" id))))

(ert-deftest satan-memory-store/parse-pg-array-empty ()
  (should (null (satan-db-parse-pg-array nil)))
  (should (null (satan-db-parse-pg-array "")))
  (should (null (satan-db-parse-pg-array "{}"))))

(ert-deftest satan-memory-store/parse-pg-array-simple ()
  (should (equal (satan-db-parse-pg-array "{app:firefox,mode:motd}")
                 '("app:firefox" "mode:motd"))))

(ert-deftest satan-memory-store/format-pg-array ()
  (should (equal (satan-memory-store--format-pg-array
                  '("app:firefox" "mode:motd"))
                 "{app:firefox,mode:motd}")))
...
(defun satan-memory-store-test--basic-mark (&optional id)
...
(ert-deftest satan-memory-store/mark-returns-trace-id ()
  (satan-memory-store-test--with-db
   (pcase (satan-memory-store-test--basic-mark)
     (`(ok . ,tid) (should (equal tid "20260519T100000-test01")))
     (err (ert-fail (format "mark failed: %S" err))))))

(ert-deftest satan-memory-store/show-round-trip ()
  (satan-memory-store-test--with-db
   (satan-memory-store-test--basic-mark)
   (pcase (satan-memory-store-show "20260519T100000-test01")
     (`(ok . ,payload)
...
(ert-deftest satan-memory-store/show-missing-trace-id ()
  (satan-memory-store-test--with-db
   (pcase (satan-memory-store-show "20260519T100000-absent")
     (`(ok . nil) t)
...
(ert-deftest satan-memory-store/resonate-empty-cue ()
  (satan-memory-store-test--with-db
   (should (equal (satan-memory-store-resonate :cue-handles nil)
                  (cons 'ok nil)))))

(ert-deftest satan-memory-store/resonate-finds-mark ()
  (satan-memory-store-test--with-db
   (satan-memory-store-test--basic-mark)
   (pcase (satan-memory-store-resonate :cue-handles '("app:firefox"))
     (`(ok . ,matches)
...
(ert-deftest satan-memory-store/resonate-returns-payload ()
...
(ert-deftest satan-memory-store/resonate-collapses-payload-whitespace ()
...
(ert-deftest satan-memory-store/resonate-orders-by-score ()
  (satan-memory-store-test--with-db
   ...
   (pcase (satan-memory-store-resonate
...
(ert-deftest satan-memory-store/resonate-respects-limit ()
  (satan-memory-store-test--with-db
   (satan-memory-store-mark
...
(ert-deftest satan-memory-store/resonate-kind-filter ()
  (satan-memory-store-test--with-db
   (satan-memory-store-mark
...
(ert-deftest satan-memory-store/bough-node-zero-weight ()
...
(ert-deftest satan-memory-store/outcome-invariant-rejects ()
  (satan-memory-store-test--with-db
   (let ((result (satan-memory-store-mark
...
(ert-deftest satan-memory-store/outcome-invariant-accepts-when-handle-present ()
  (satan-memory-store-test--with-db
   (pcase (satan-memory-store-mark
...
(ert-deftest satan-memory-store/origin-admission ()
  ...
  (satan-memory-store-test--with-db
   ...
   (satan-memory-store-mark
    ...
    :handles (list (list :handle "mode:motd"
                         :source (list :rule_id "ctx"))))
   (pcase (satan-memory-store-resonate :cue-handles '("mode:motd"))
     (`(ok . ,matches)
      (should (= 2 (length matches))))
     (err (ert-fail (format "resonate failed: %S" err))))))
...
(ert-deftest satan-memory-store/links-round-trip ()
  (satan-memory-store-test--with-db
   ;; predecessor trace
   (satan-memory-store-mark
   ...
   (pcase (satan-memory-store-show "20260519T100000-link01")
     (`(ok . ,payload)
...
(ert-deftest satan-memory-store/recent-empty ()
  (satan-memory-store-test--with-db
   (should (equal (satan-memory-store-recent) (cons 'ok nil)))))

(ert-deftest satan-memory-store/recent-orders-newest-first ()
  (satan-memory-store-test--with-db
   (satan-memory-store-mark
...
(ert-deftest satan-memory-store/recent-respects-limit ()
  (satan-memory-store-test--with-db
   (dotimes (i 3)
     (satan-memory-store-mark
      ...
      :handles (list (list :handle "app:firefox"
                           :source (list :rule_id "r" :origin "observed")))))
   (pcase (satan-memory-store-recent :limit 2)
     (`(ok . ,rows) (should (= 2 (length rows))))
     (err (ert-fail (format "recent failed: %S" err))))))

(ert-deftest satan-memory-store/recent-kind-filter ()
  (satan-memory-store-test--with-db
   (satan-memory-store-mark
    :trace-id "20260519T100000-obs001"
    :kind "observation" :trace-origin "llm_mark" :source "t"
   ...
   (satan-memory-store-mark
    ...
    :handles (list (list :handle "app:firefox"
                         :source (list :rule_id "r" :origin "observed"))))
   (pcase (satan-memory-store-recent :kinds '("intervention"))
     (`(ok . ,rows)
      (should (= 1 (length rows)))
      (should (equal (plist-get (car rows) :kind) "intervention")))
     (err (ert-fail (format "recent failed: %S" err))))))

(ert-deftest satan-memory-store/recent-returns-full-payload ()
  ...
  (satan-memory-store-test--with-db
   (let ((long (make-string 400 ?x)))
     (satan-memory-store-mark
      ...
      :handles (list (list :handle "app:firefox"
                           :source (list :rule_id "r" :origin "observed"))))
     (pcase (satan-memory-store-recent :limit 1)
       (`(ok . ,rows)
        (should (= 400 (length (plist-get (car rows) :payload))))
        (should (equal long (plist-get (car rows) :payload))))
       (err (ert-fail (format "recent failed: %S" err)))))))

(provide 'satan-memory-store-test)
;;; satan-memory-store-test.el ends here
