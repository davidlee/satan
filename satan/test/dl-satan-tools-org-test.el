...
(defconst dl-satan-org-test--ctx
...
(ert-deftest dl-satan-org/proposal-stage-surfaces-intervention-id ()
  ...
  (let* ((tmp (make-temp-file "satan-proposals-" t))
    ...
    (unwind-protect
        (dl-satan-org-test--with-stubs
          (let ((res (dl-satan-tool/proposal-stage
                      '(:title "Refactor X" :body "consider Y")
                      dl-satan-org-test--ctx)))
...
(ert-deftest dl-satan-org/proposal-stage-intervention-args-shape ()
  "Handler passes §3.3 defaults into `dl-satan-intervention-create'."
  (let* ((tmp (make-temp-file "satan-proposals-" t))
         (dl-satan-proposals-dir tmp))
    (unwind-protect
        (dl-satan-org-test--with-stubs
          (dl-satan-tool/proposal-stage
           '(:title "Refactor X" :body "consider Y")
           dl-satan-org-test--ctx)
          (let ((args (car dl-satan-org-test--captured)))
            (should args)
