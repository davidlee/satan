;;; dl-satan-patch-adapter.el --- patch-agent adapter protocol -*- lexical-binding: t; -*-
...
;;
;;   JOB-SPEC    ::  the row plist returned by `dl-satan-patch-store-get'.
;;                   `:worktree_path' is already on disk; the adapter must
;;                   set `default-directory' to it and never wander out.
;;   INPUT       ::  the plist produced by `dl-satan-patch-prompt-build':
...
;; `dl-satan-patch-adapter-register'.  The runner dispatches by name.

(require 'cl-lib)
(require 'subr-x)

(defvar dl-satan-patch-adapters nil
...
(defun dl-satan-patch-adapter-register (name fn)
  "Register adapter FN under NAME, replacing any prior entry."
  (setq dl-satan-patch-adapters
        (cons (cons name fn)
              (cl-remove name dl-satan-patch-adapters
                         :key #'car :test #'equal))))

(defun dl-satan-patch-adapter-lookup (name)
  "Return the adapter function registered under NAME, or nil."
  (cdr (assoc name dl-satan-patch-adapters)))

(cl-defun dl-satan-patch-adapter-invoke
...
are forwarded to the adapter; see the module commentary."
  (let ((fn (dl-satan-patch-adapter-lookup name)))
    (unless fn
      (error "dl-satan-patch-adapter: no adapter registered for %S" name))
