;;; satan-motive-test.el --- Phase 3 motive ert -*- lexical-binding: t; -*-
...
(defconst satan-motive-test--well-formed
...
(defconst satan-motive-test--well-formed-cwd
  "* test: docs-after-error
  Docs after terminal error often substitute orientation for contact.
  :cue: project:emacs.d surface_transition:terminal->browser domain_kind:docs
  :cooldown_s: 1800
...
* ruminations
  ...
  - 2026-05-19  patch jobs accepted more when directive cites file path
"
  "Like `satan-motive-test--well-formed', but docs-after-error carries
...
(defconst satan-motive-test--ceiling
  "* test: pestered
...
(defconst satan-motive-test--malformed-cue
  "* test: bad
...
(defconst satan-motive-test--ctx-only-cue
  "* test: too-generic
...
(defconst satan-motive-test--with-project-cwd
...
(defconst satan-motive-test--four-actives
  "* test: a
  :cue: app:firefox
  :cooldown_s: 1800
* test: b
...
(defun satan-motive-test--n-ruminations (n)
...
(ert-deftest satan-motive/parse-accepts-all-footer-fields ()
  (let* ((parsed (satan-motive-parse satan-motive-test--well-formed))
...
(ert-deftest satan-motive/parse-extracts-ruminations ()
  (let* ((parsed (satan-motive-parse satan-motive-test--well-formed))
...
(ert-deftest satan-motive/parse-rejects-ceiling-field ()
...
itself is still returned (file-tolerated) for diagnostic rendering."
  (let* ((parsed (satan-motive-parse satan-motive-test--ceiling))
...
(ert-deftest satan-motive/parse-marks-missing-cue-dormant ()
  "A8 — a motive without a `:cue:' is dormant (file-tolerated,
capsule-invisible, observer-skipped)."
  (let* ((text "* test: no-cue\n  Prose only.\n  :cooldown_s: 1800\n")
         (parsed (satan-motive-parse text))
         (m (car (plist-get parsed :motives))))
    (should (plist-get m :dormant))
    (should (eq :missing-cue (plist-get m :dormant_reason)))))

(ert-deftest satan-motive/parse-marks-malformed-cue-dormant ()
  (let* ((parsed (satan-motive-parse satan-motive-test--malformed-cue))
         (m (car (plist-get parsed :motives))))
    (should (plist-get m :dormant))
    (should (eq :malformed-cue (plist-get m :dormant_reason)))))

(ert-deftest satan-motive/parse-marks-ctx-only-cue-dormant ()
...
handle the motive triggers on every tick — defeats cooldown."
  (let* ((parsed (satan-motive-parse satan-motive-test--ctx-only-cue))
         (m (car (plist-get parsed :motives))))
    (should (plist-get m :dormant))
...
(ert-deftest satan-motive/read-missing-file-returns-empty ()
  (let* ((tmp (make-temp-file "satan-motive-missing-"))
         (_ (delete-file tmp))
         (parsed (satan-motive-read tmp)))
    (should (null (plist-get parsed :motives)))))
...
(ert-deftest satan-motive/parse-extracts-project-cwd ()
...
predicate cannot fire path-scoped sub-predicates without it."
  (let* ((parsed (satan-motive-parse
                  satan-motive-test--with-project-cwd))
         (m (car (plist-get parsed :motives))))
    (should (equal (expand-file-name "~/.emacs.d")
                   (plist-get m :project_cwd)))
    (should-not (plist-get m :dormant))))

(ert-deftest satan-motive/parse-absent-project-cwd-is-nil ()
...
motive's cwd) are skipped for them."
  (let* ((parsed (satan-motive-parse
                  satan-motive-test--well-formed))
         (m (car (plist-get parsed :motives))))
    (should (null (plist-get m :project_cwd)))))

(ert-deftest satan-motive/parse-empty-project-cwd-is-nil ()
...
string from being passed to file-name predicates as `/'."
  (let* ((text "* test: x\n  :cue: app:firefox\n  :cooldown_s: 1800\n  :project_cwd: \n")
         (m (car (plist-get (satan-motive-parse text) :motives))))
    (should (null (plist-get m :project_cwd)))))

(ert-deftest satan-motive/write-accepts-project-cwd ()
  "Phase 5.0 — the write-side guard accepts the new field."
  (should (null (satan-motive-validate-for-write
                 satan-motive-test--with-project-cwd))))

(ert-deftest satan-motive/render-block-omits-project-cwd ()
...
and keeps the cooldown_s/worked_count line stable)."
  (let* ((framing '(("motive_block_header" . "# Motive")))
         (block (satan-motive-render-block
                 framing
                 (satan-motive-parse
                  satan-motive-test--with-project-cwd))))
    (should block)
...
(ert-deftest satan-motive/worked-count-does-not-reorder ()
...
* test: beta
  ...
  :worked_count: 0
")
         (block-low (satan-motive-render-block
                     framing (satan-motive-parse low)))
         (block-high (satan-motive-render-block
                      framing (satan-motive-parse high)))
...
(ert-deftest satan-motive/render-block-shape ()
  (let* ((framing '(("motive_block_header" . "# Motive")))
         (block (satan-motive-render-block
                 framing
                 (satan-motive-parse satan-motive-test--well-formed))))
...
(ert-deftest satan-motive/render-block-omits-dormant ()
  "Dormant motives never render — §S3 says they are file-tolerated
but capsule-invisible."
  (let* ((framing '(("motive_block_header" . "# Motive")))
         (text (concat satan-motive-test--well-formed
                       satan-motive-test--malformed-cue))
         (block (satan-motive-render-block
                 framing (satan-motive-parse text)))
...
(ert-deftest satan-motive/render-block-omits-when-no-active ()
...
(ert-deftest satan-motive/render-block-without-framing-key-yields-nil ()
  "Mind owns the header text; absent key suppresses the section."
  (let* ((framing '(("percept_block_header" . "# Percept")))
         (parsed (satan-motive-parse satan-motive-test--well-formed)))
    (should (null (satan-motive-render-block framing parsed)))))

;; ---------------------------------------------------------------------
;; Render — cooldown floor (Phase 6, §S4)
;; ---------------------------------------------------------------------

(defconst satan-motive-test--cooldown-fixture
  "* test: docs-after-error
  Docs after terminal error often substitute orientation for contact.
  :cue: project:emacs.d surface_transition:terminal->browser domain_kind:docs
  :cooldown_s: 1800
...
(ert-deftest satan-motive/render-block-cooling-down-annotates-header ()
...
intact.  Baseline last_intervention_at + 12m → 18m remaining of 30m."
  (let* ((framing '(("motive_block_header" . "# Motive")))
         (parsed (satan-motive-parse satan-motive-test--cooldown-fixture))
         (now    "2026-05-23T10:12:00+1000")
         (block  (satan-motive-render-block framing parsed now)))
    (should (cl-some (lambda (l)
                       (string-match-p
...
(ert-deftest satan-motive/render-block-cooldown-elapsed-renders-actionable ()
...
(ert-deftest satan-motive/render-block-no-last-intervention-renders-actionable ()
...
(ert-deftest satan-motive/render-block-no-cooldown-renders-actionable ()
...
(ert-deftest satan-motive/write-rejects-too-many-actives ()
  (let ((err (satan-motive-validate-for-write
              satan-motive-test--four-actives)))
    ...
    (should (string-match-p "too many active motives: limit 3, got 4"
                            (satan-motive-format-write-error err)))))

(ert-deftest satan-motive/write-rejects-too-many-ruminations ()
  (let* ((text (satan-motive-test--n-ruminations 11))
         (err (satan-motive-validate-for-write text)))
    (should (eq :too-many-ruminations (plist-get err :bound)))
    (should (= 10 (plist-get err :limit)))
    (should (= 11 (plist-get err :got)))))

(ert-deftest satan-motive/write-rejects-ceiling-field ()
  (let ((err (satan-motive-validate-for-write
              satan-motive-test--ceiling)))
...
(ert-deftest satan-motive/write-rejects-malformed-cue ()
...
(ert-deftest satan-motive/write-rejects-ctx-only-cue ()
...
the author would not see why."
  (let ((err (satan-motive-validate-for-write
              satan-motive-test--ctx-only-cue)))
    (should (eq :invalid-cue (plist-get err :bound)))
    (should (eq :no-sensor-handle (plist-get err :reason)))))

(ert-deftest satan-motive/write-accepts-well-formed ()
  (should (null (satan-motive-validate-for-write
                 satan-motive-test--well-formed))))

(ert-deftest satan-motive/write-accepts-exactly-three-actives ()
  "Boundary: 3 actives + 10 ruminations is the limit, not over it."
  (let* ((three "* test: a\n  :cue: app:firefox\n  :cooldown_s: 1800\n* test: b\n  :cue: app:firefox\n  :cooldown_s: 1800\n* test: c\n  :cue: app:firefox\n  :cooldown_s: 1800\n")
         (ten (satan-motive-test--n-ruminations 10)))
    (should (null (satan-motive-validate-for-write three)))
    (should (null (satan-motive-validate-for-write ten)))))

(ert-deftest satan-motive/write-accepts-missing-cue ()
...
The write path tolerates it; the read path renders it dormant."
  (let ((text "* test: drafting\n  Prose only.\n  :cooldown_s: 1800\n"))
    (should (null (satan-motive-validate-for-write text)))))

;; ---------------------------------------------------------------------
;; A7 precedence — first breach in `satan-motive-bound-precedence' wins
;; ---------------------------------------------------------------------

(ert-deftest satan-motive/bound-precedence-shape ()
  "Document-visible precedence constant exists with the expected
order — forbidden-field > count caps > invalid-cue (A7)."
  (should (equal satan-motive-bound-precedence
                 '(:forbidden-field :too-many-active
                   :too-many-ruminations :invalid-cue))))

(ert-deftest satan-motive/write-precedence-forbidden-beats-count ()
...
field through a fix-the-count edit."
  (let* ((bad (concat "* test: a\n  :cue: app:firefox\n  :ceiling: 5\n"
                      ...
                      "* test: c\n  :cue: app:firefox\n  :cooldown_s: 1800\n"
                      "* test: d\n  :cue: app:firefox\n  :cooldown_s: 1800\n"))
         (err (satan-motive-validate-for-write bad)))
    (should (eq :forbidden-field (plist-get err :bound)))))

(ert-deftest satan-motive/write-precedence-count-beats-invalid-cue ()
...
(ert-deftest satan-motive/write-precedence-active-beats-ruminations ()
...
(ert-deftest satan-motive/write-invalid-cue-reports-first-bad-motive ()
...
(ert-deftest satan-motive/format-error-names-every-bound ()
...
(defmacro satan-motive-test--with-framing (tmp-sym &rest body)
...
(ert-deftest satan-motive/capsule-renders-motive-between-resonance-and-today ()
...
between `# Resonance' and `# Today (raw)' per §S1 sequence."
  (satan-motive-test--with-framing tmp
    (let* ((spec (list :name "motd"
           ...
           (prepare (list :run_id "rid-x"
                          ...
                          (satan-motive-parse
                           satan-motive-test--well-formed)))
           (bundle (satan-context-motd spec prepare))
...
(ert-deftest satan-motive/capsule-omits-motive-when-empty ()
...
(ert-deftest satan-motive/capsule-omits-when-only-dormant ()
...
(ert-deftest satan-motive/with-prepare-mirrors-motive-slot ()
  (let* ((parsed (satan-motive-parse
                  satan-motive-test--well-formed))
         (prepare (list :run_id "rid"
                        ...
                        :motive parsed))
         (bundle (satan-context--with-prepare '() prepare)))
...
(defmacro satan-motive-test--with-tmp-file (var text &rest body)
...
(defun satan-motive-test--read (path)
  (with-temp-buffer
    (let ((coding-system-for-read 'utf-8))
      (insert-file-contents path))
    (buffer-string)))

(ert-deftest satan-motive/touch-footer-replaces-worked-count-in-place ()
  "Existing `:worked_count:' line is replaced; indentation preserved."
  (satan-motive-test--with-tmp-file path
      satan-motive-test--well-formed
    (should (satan-motive-touch-footer
             "docs-after-error" 7 "2026-05-22T10:30:00+1000" path))
    (let ((out (satan-motive-test--read path)))
...
(ert-deftest satan-motive/touch-footer-replaces-last-intervention-at ()
  "Existing `:last_intervention_at:' line gets the new ISO."
  (satan-motive-test--with-tmp-file path
      satan-motive-test--well-formed
    (satan-motive-touch-footer
...
(ert-deftest satan-motive/touch-footer-appends-missing-fields ()
...
(ert-deftest satan-motive/touch-footer-preserves-prose-and-ruminations ()
...
(ert-deftest satan-motive/touch-footer-only-mutates-target-section ()
...
(ert-deftest satan-motive/touch-footer-unknown-id-returns-nil ()
  "Unknown motive id yields nil and leaves the file untouched."
  (satan-motive-test--with-tmp-file path
      satan-motive-test--well-formed
    (let ((before (satan-motive-test--read path)))
      (should-not (satan-motive-touch-footer
                   "nonexistent-motive" 1 "2026-05-22T10:30:00+1000" path))
      (should (equal before (satan-motive-test--read path))))))

(ert-deftest satan-motive/touch-footer-missing-file-returns-nil ()
...
(ert-deftest satan-motive/touch-footer-roundtrips-through-parse ()
  "After touch-footer, `satan-motive-parse' on the file yields the
expected updated values."
  (satan-motive-test--with-tmp-file path
      satan-motive-test--well-formed
    (satan-motive-touch-footer
     "docs-after-error" 11 "2026-05-22T10:30:00+1000" path)
    (let* ((parsed (satan-motive-parse (satan-motive-test--read path)))
...
(ert-deftest satan-motive/touch-footer-no-trailing-newline-handled ()
  "Files without a trailing newline still rewrite cleanly — common
on hand-edited org files."
  (satan-motive-test--with-tmp-file path
      (substring satan-motive-test--well-formed 0
                 (1- (length satan-motive-test--well-formed)))
    (should (satan-motive-touch-footer
             "docs-after-error" 3 "2026-05-22T10:30:00+1000" path))
    (let ((parsed (satan-motive-parse
                   (satan-motive-test--read path))))
      (should (= 3 (plist-get (car (plist-get parsed :motives))
                              :worked_count))))))

(provide 'satan-motive-test)
;;; satan-motive-test.el ends here
