;;; satan-observer.el --- SATAN outcome observer (Phase 5) -*- lexical-binding: t; -*-
...
;; `satan-observer-classify' (§S5 P1–P4), and persists the verdict
;; through `satan-intervention-classify' (audit-event +
...
;; `satan-intervention-classify' which writes an `intervention.
...
(defun satan-observer--run-dir-for (run-id &optional runs-dir)
...
(defun satan-observer--projection-to-classifier-plist (row &optional runs-dir)
  "Normalise a projection ROW to the plist the classifier consumes.
ROW is a `satan-intervention-pending' entry (per
`satan-intervention--row-to-intervention').  Adds the slots the
...
verdict-persister carries through to the trace metadata."
  (let* ((run-id (plist-get row :run_id))
         (run-dir (satan-observer--run-dir-for run-id runs-dir))
         (iv-id (plist-get row :intervention_id)))
    (append row
            (list :run_dir run-dir
                  :intervention_emitted_at (plist-get row :ts)
                  :applied_index (satan-observer--applied-index-from-id
                                  iv-id)))))

(defun satan-observer-pending (now &optional runs-dir db)
  "Return classifier-ready intervention plists matured by NOW.
NOW is an ISO8601 string accepted by PostgreSQL's `timestamptz'
parser; RUNS-DIR overrides `satan-runs-dir'; DB overrides the
migrate database.  Thin wrapper over `satan-intervention-pending'
that resolves each row's `:run_dir' and shapes the plist for the
classifier."
  (mapcar (lambda (row)
            (satan-observer--projection-to-classifier-plist row runs-dir))
          (satan-intervention-pending now db)))

;; ---------------------------------------------------------------------
;; Verdict persistence
;; ---------------------------------------------------------------------

(defun satan-observer--motive-handle-rows (motive)
...
(defun satan-observer--persist-positive (intervention motive verdict now opts)
...
N)'."
  (let* ((motive-path (or (plist-get opts :motive-path)
         ...
         (handles (satan-observer--motive-handle-rows motive))
         ...
         (trace-result
          (funcall mark-fn
                   ...
                   :observed-end-at
                   (satan-observer--window-end-iso intervention)
                   :payload payload
...
(defun satan-observer--verdict-classify-args (intervention verdict now)
...
`:time_now' ISO string."
  (let* ((classification (plist-get verdict :classification))
         ...
         (common (list :classification
                       (satan-observer--keyword-to-string classification)
                       :confidence
                       (satan-observer--keyword-to-string confidence)
                       :maturity
                       (satan-observer--keyword-to-string
                        (or (plist-get verdict :maturity) :mature))
                       :next-revisit-at
                       (satan-observer--next-revisit-iso intervention)
                       :source "auto"
                       :classified-at now))
         (evidence
          (pcase classification
            (:worked
             (list :source_events '()
                   :predicates
                   (mapcar #'satan-observer--keyword-to-string
            ...
            (_
             (list :source_events '()
                   :reason (or (satan-observer--keyword-to-string
                                (plist-get verdict :reason))
                               :null))))))
    (append common (list :evidence evidence))))

(defun satan-observer-persist-verdict
...
`satan-motive-touch-footer'; an observation / auto_rule trace via
`satan-memory-store-mark'.

Every verdict — `:worked' or otherwise — is committed through
`satan-intervention-classify', which emits an `intervention.
...
OPTS forwards to the lower-level writers (used in tests):
  ...
  :ctx                tool-ctx plist for `satan-intervention-classify'
...
Returns plist:
  ...
  (let* ((opts (or opts '()))
         ...
         (result (when positive
                   (satan-observer--persist-positive
                    intervention motive verdict now opts)))
         (ctx (plist-get opts :ctx))
         (db (plist-get opts :db))
         (classify-kw (satan-observer--verdict-classify-args
                       intervention verdict now))
         (event (apply #'satan-intervention-classify
                       :ctx ctx
...
Sequence:
  ...
  2. Get pending interventions from the projection
     (`satan-intervention-pending'), normalised with `:run_dir' +
     `:intervention_emitted_at' + `:applied_index'.
  3. For each pending intervention, run
     `satan-observer-classify-for-motives' then
     `satan-observer-persist-verdict' (writes the audit event +
...
Returns a summary plist for audit visibility:
  ...
  (let* ((opts (or opts '()))
    ...
    ;; global-attribute enqueue inside `satan-intervention-classify').
