...
Reference for `satan-observer-classify` (§S5, outcome-semantics §2).
...
| present → `:stale` | — | returns nil; caller skips persist (§3, §6.3 — auto re-pass forbidden past stale cutoff). `satan-intervention-pending` excludes stale rows in SQL, so this branch should be reachable only when the SQL window and the elisp `maturity-state` disagree. **It did, for months** — `psql -A` dumps `timestamptz` as the space-separated `YYYY-MM-DD HH:MM:SS+00` form, which `date-to-time` mis-parsed ~1.5 days into the past, so every in-window intervention read `:stale` here and was skipped (the cold outcome pipeline). Closed 2026-05-29 by normalizing the cell at the DB-row boundary (`satan-intervention--normalize-pg-timestamp`); this branch is now genuinely defence-in-depth. |
...
5. **None fire** → `satan-observer-classify-negative` →
...
- Final verdict passes through `satan-observer--assert-auto-classification` so auto callers can never construct `:harmful` / `:contradicted` (manual-only per §2 invariants 1 + 2).
