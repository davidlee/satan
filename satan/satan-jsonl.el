;;; satan-jsonl.el --- Line-buffered JSONL filter + writer -*- lexical-binding: t; -*-
...
(defun satan-jsonl--alist-key-to-keyword (k)
...
through untouched."
  (cond
   ((vectorp v)
    (vconcat (mapcar #'satan-jsonl-prepare (append v nil))))
   ((satan-jsonl--plist-p v)
    (cl-loop for (k val) on v by #'cddr
             append (list k (satan-jsonl-prepare val))))
   ((satan-jsonl--alist-p v)
    (cl-loop for (k . val) in v
             append (list (satan-jsonl--alist-key-to-keyword k)
                          (satan-jsonl-prepare val))))
   ((and (consp v) (listp (cdr v)))
    (vconcat (mapcar #'satan-jsonl-prepare v)))
...
(defun satan-jsonl-make-filter (on-object on-error)
...
(defun satan-jsonl-send (proc obj)
  "Serialize OBJ to JSON and send one newline-terminated line to PROC stdin."
  (let ((line (json-serialize (satan-jsonl-prepare obj)
                              :null-object :null :false-object :false)))
    (process-send-string proc (concat line "\n"))))

(cl-defun satan-jsonl-read-file (path &key null-object)
