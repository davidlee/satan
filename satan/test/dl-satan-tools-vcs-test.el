;;; dl-satan-tools-vcs-test.el --- vcs_log ert -*- lexical-binding: t; -*-

;; Run from CLI:
;;   emacs --batch \
;;     -L ~/.emacs.d/core -L ~/.emacs.d/satan -L ~/.emacs.d/satan/test \
;;     -l dl-satan-tools-vcs-test.el -f ert-run-tests-batch-and-exit

(require 'ert)
(require 'cl-lib)
(require 'dl-satan-tools-vcs)

(defmacro dl-satan-tools-vcs-test--in-repo (var &rest body)
...
(ert-deftest dl-satan-tools-vcs/log-ok-pwd-independent ()
  "Returns history for an abs-path repo WITHOUT relying on
`default-directory' (deliberately pointed elsewhere)."
  (skip-unless (executable-find "git"))
  (dl-satan-tools-vcs-test--in-repo repo
    (let* ((default-directory temporary-file-directory)
           (result (dl-satan-tool/vcs-log (list :repo repo) nil)))
...
(ert-deftest dl-satan-tools-vcs/not-found ()
  (let ((result (dl-satan-tool/vcs-log
                 (list :repo "/no/such/repo/anywhere-xyz123") nil)))
    (should (eq 'error (car result)))
    (should (string-match-p "repo not found" (cdr result)))))

(ert-deftest dl-satan-tools-vcs/not-a-git-repo ()
  (skip-unless (executable-find "git"))
  (let ((tmp (make-temp-file "satan-vcs-nogit-" t)))
    (unwind-protect
        (let ((result (dl-satan-tool/vcs-log (list :repo tmp) nil)))
          (should (eq 'error (car result)))
          (should (string-match-p "not a git repo" (cdr result))))
      (delete-directory tmp t))))

(ert-deftest dl-satan-tools-vcs/limit-clamp ()
  (should (= 20 (dl-satan-tools-vcs--clamp-limit nil)))
  (should (= 1 (dl-satan-tools-vcs--clamp-limit 0)))
  (should (= 200 (dl-satan-tools-vcs--clamp-limit 9999)))
  (should (= 5 (dl-satan-tools-vcs--clamp-limit 5))))

(provide 'dl-satan-tools-vcs-test)
;;; dl-satan-tools-vcs-test.el ends here
