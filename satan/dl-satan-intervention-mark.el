;;; dl-satan-intervention-mark.el --- manual override path for intervention outcomes -*- lexical-binding: t; -*-

;; T1.5b PR 4 — interactive entry to the §7.1 manual-mark workflow.
;; The notes-directive entry lives in dl-satan-tools-atsatan.el; both
;; surfaces route through `dl-satan-intervention-write-manual-outcome'
;; in dl-satan-intervention.el.
;;
;; Commands:
;;   M-x my/satan-mark-intervention-harmful
;;   M-x my/satan-mark-intervention-contradicted
;;
...
(require 'dl-satan-observer-classify)  ; maturity-state util

;; Broker dependency is soft — `dl-satan-broker-locate-run-dir' is loaded
;; into the live emacs daemon at session start.  Requiring it here would
;; pull `dl-satan-tools-org' (and its denote chain) into ert batch runs
;; that don't need them.
(declare-function dl-satan-broker-locate-run-dir "dl-satan-broker")

(defconst dl-satan-intervention-mark--confidences
  '("low" "medium" "high")
  "Confidence enum offered by the interactive command (§4).")

(defun dl-satan-intervention-mark--run-id-of (iv-id)
...
(defun dl-satan-intervention-mark--now-iso ()
  "Return current time as the ISO8601 form the audit boundary expects."
  (format-time-string "%Y-%m-%dT%H:%M:%S%z"))

(defun dl-satan-intervention-mark--next-revisit-at (intervention)
...
(defun dl-satan-intervention-mark--read-iv-id (include-stale)
  "Prompt for an intervention id; return the bare id string.
INCLUDE-STALE forwards to `dl-satan-intervention-recent'."
  (let* ((now (dl-satan-intervention-mark--now-iso))
         (ivs (dl-satan-intervention-recent
               now :include-stale include-stale :limit 50))
         (table (mapcar (lambda (iv)
                          (cons (dl-satan-intervention-mark--candidate-label iv)
...
(defun dl-satan-intervention-mark--default-evidence-pointer ()
...
(defun dl-satan-intervention-mark--read-confidence ()
  (completing-read "Confidence: "
                   dl-satan-intervention-mark--confidences
                   nil t nil nil "medium"))

(defun dl-satan-intervention-mark--build-ctx (run-id audit now)
...
(defun dl-satan-intervention-mark--dispatch (classification include-stale)
...
toggles whether :stale interventions appear in the completion list."
  (let* ((iv-id (dl-satan-intervention-mark--read-iv-id include-stale))
         (lookup (dl-satan-intervention-lookup iv-id))
    ...
    (let* ((reason (read-string "Reason: "))
           (default-evi (dl-satan-intervention-mark--default-evidence-pointer))
           ...
           (confidence (dl-satan-intervention-mark--read-confidence))
           (notes (let ((s (read-string "Notes (optional): ")))
                    (if (string-empty-p s) nil s)))
           (now (dl-satan-intervention-mark--now-iso))
           (run-id (dl-satan-intervention-mark--run-id-of iv-id))
           (run-dir (dl-satan-broker-locate-run-dir run-id))
           (_ (unless run-dir
                (user-error "no run-dir on disk for %s" run-id)))
           (audit (dl-satan-audit-reopen run-dir))
           (ctx (dl-satan-intervention-mark--build-ctx run-id audit now))
           (maturity (dl-satan-intervention-mark--maturity iv now))
           (next-revisit-at
            (dl-satan-intervention-mark--next-revisit-at iv))
...
;;;###autoload
(defun my/satan-mark-intervention-harmful (&optional include-stale)
...
With prefix arg INCLUDE-STALE, allow choosing a `:stale' intervention."
  (interactive "P")
  (dl-satan-intervention-mark--dispatch "harmful" include-stale))

;;;###autoload
(defun my/satan-mark-intervention-contradicted (&optional include-stale)
  "Manually mark an intervention as `:contradicted' (outcome-semantics §7.1).
With prefix arg INCLUDE-STALE, allow choosing a `:stale' intervention."
  (interactive "P")
  (dl-satan-intervention-mark--dispatch "contradicted" include-stale))

(provide 'dl-satan-intervention-mark)
;;; dl-satan-intervention-mark.el ends here
