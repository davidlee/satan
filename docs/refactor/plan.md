...
|---|---|---|---|---|
| [T1](T1-observer-split.md)         | Observer file-split                 | merged      | —              | done — classifier extracted to `satan-observer-classify.el` |
...
| [T2](T2-pre-spawn.md)              | Pre-spawn pipeline extraction       | not-started | —              | `satan-pre-spawn.el` + cutover |
| [T3](T3-capsule-registry.md)       | Capsule render registry             | not-started | —              | (Path A gated; awaits rendering pain) |
| [T4](T4-modes-field.md)            | Drop tool-spec `:modes` field       | merged      | —              | done — `:modes' stripped from all tool specs; `satan-mode-check-tool-references' enforces |
...
- **Rollback switches.** T7 + attributes tranche introduce `satan-intervention-recording-enabled` + `satan-attribute-updates-enabled` defcustoms (CODE_REVIEW.md §6 Q9). Wire in T7 PR sequence.
...
2. `satan-memory-writer.el` facade — 5 callers across 3 files; below facade threshold.
