;;; dl-satan-ingest-cursor-test.el --- ert tests for dl-satan-ingest-cursor -*- lexical-binding: t; -*-
...
(require 'dl-satan-tools-content-test nil t) ; fixture macros (write-article-jsonl etc.)
(require 'dl-satan-sensor-content)
(require 'dl-satan-ingest-cursor)

;; --- Fixture helpers -------------------------------------------

(defmacro dl-satan-ingest-cursor-test--with-env (&rest body)
  "Run BODY with activity dir, content dir, and cursor state file all in temps.
Binds `dl-satan-tools-activity-dir', `dl-satan-tools-content-dir', and
`dl-satan-ingest-cursor-state-file' to isolated temp trees.
Also binds `dl-satan-tools-descriptions-dir' so tool schema assembly works."
  (declare (indent 0))
  `(let* ((act-dir   (make-temp-file "satan-cursor-act-" t))
          ...
          (dl-satan-tools-activity-dir act-dir)
          ...
          (dl-satan-ingest-cursor-state-file cursor-f))
     ...
     (unwind-protect (progn ,@body)
       ...
       (ignore-errors (delete-directory dl-satan-tools-content-dir t))
       (ignore-errors (delete-directory desc-dir t))
       (ignore-errors (delete-file cursor-f)))))

(defun dl-satan-ingest-cursor-test--today ()
  (format-time-string "%Y-%m-%d"))

(defun dl-satan-ingest-cursor-test--write-segment (dir kind rows)
  "Write ROWS (list of plists) as today's KIND segments JSONL under DIR."
  (let ((path (expand-file-name
               (format "segments/%s-%s.jsonl"
                       kind (dl-satan-ingest-cursor-test--today))
               dir)))
    (with-temp-file path
      (dolist (r rows)
        (insert (json-serialize (dl-satan-jsonl-prepare r)
                                :null-object :null :false-object :false))
        (insert "\n")))))

(defun dl-satan-ingest-cursor-test--write-articles (content-dir rows)
...
(defun dl-satan-ingest-cursor-test--seed-cursor (cursor-f plist)
...
(defun dl-satan-ingest-cursor-test--seg (end-ts)
...
(defun dl-satan-ingest-cursor-test--article (captured-at)
...
(ert-deftest dl-satan-ingest-cursor/advance-writes-head-per-source ()
  "Advance writes each source's head TS to the cursor state file."
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
          (content dl-satan-tools-content-dir)
          (cfile dl-satan-ingest-cursor-state-file))
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T09:30:00+10:00")
             (dl-satan-ingest-cursor-test--seg "2026-06-10T09:45:00+10:00")))
      (dl-satan-ingest-cursor-test--write-browser-and-content
       today content
...
(defun dl-satan-ingest-cursor-test--write-browser-and-content
    (act-dir content-dir browser-ts content-ts)
  "Seed today's browser segment and articles.jsonl with single rows."
  (dl-satan-ingest-cursor-test--write-segment
   act-dir "browser"
   (list (dl-satan-ingest-cursor-test--seg browser-ts)))
  (dl-satan-ingest-cursor-test--write-articles
   content-dir
   (list (dl-satan-ingest-cursor-test--article content-ts))))

(ert-deftest dl-satan-ingest-cursor/advance-idempotent ()
  ...
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
      ...
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T10:00:00+10:00")))
      (dl-satan-ingest-cursor-test--write-browser-and-content
       today content
       "2026-06-10T09:00:00+10:00"
       "2026-06-10T05:00:00.000Z")
      (dl-satan-ingest-cursor-advance)
      (let ((state1 (dl-satan-ingest-cursor-read)))
        (dl-satan-ingest-cursor-advance)
        (let ((state2 (dl-satan-ingest-cursor-read)))
...
(ert-deftest dl-satan-ingest-cursor/advance-does-not-regress-on-older-row ()
  ...
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
      ...
      (dl-satan-ingest-cursor-test--seed-cursor
      ...
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T09:00:00+10:00")))
      (dl-satan-ingest-cursor-test--write-browser-and-content
       today content
...
(ert-deftest dl-satan-ingest-cursor/advance-mixed-offset-focus-uses-parsed-instant ()
...
local-offset row is selected, not discarded by string< (which would rank Z lower)."
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
      ...
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T08:00:00+10:00") ; stale local
             (dl-satan-ingest-cursor-test--seg "2026-06-09T23:30:00Z")))   ; newer Z-form
      (dl-satan-ingest-cursor-test--write-browser-and-content
       today content
       "2026-06-10T09:00:00+10:00"
       "2026-06-10T05:00:00.000Z")
...
(ert-deftest dl-satan-ingest-cursor/advance-missing-cursor-initialises-to-head ()
  "When no cursor file exists, advance initialises each source to its head."
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
          (content dl-satan-tools-content-dir))
      (should-not (dl-satan-ingest-cursor-read)) ; cursor absent initially
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T09:00:00+10:00")))
      (dl-satan-ingest-cursor-test--write-browser-and-content
...
(ert-deftest dl-satan-ingest-cursor/backlog-depth-known-cursor ()
  ...
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
      ...
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T09:00:00+10:00") ; == cursor
             (dl-satan-ingest-cursor-test--seg "2026-06-10T09:30:00+10:00") ; after
             (dl-satan-ingest-cursor-test--seg "2026-06-10T10:00:00+10:00"))) ; after
      ;; 2 browser rows: 1 before cursor, 1 after
      (dl-satan-ingest-cursor-test--write-segment
       today "browser"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T07:30:00+10:00") ; before
             (dl-satan-ingest-cursor-test--seg "2026-06-10T08:30:00+10:00"))) ; after
      ;; 3 articles: 1 before cursor, 2 after
      (dl-satan-ingest-cursor-test--write-articles
       content
       (list (dl-satan-ingest-cursor-test--article "2026-06-10T02:00:00.000Z") ; before
             (dl-satan-ingest-cursor-test--article "2026-06-10T04:00:00.000Z") ; after
             (dl-satan-ingest-cursor-test--article "2026-06-10T05:00:00.000Z"))) ; after
      (let ((depth (dl-satan-ingest-cursor-backlog-depth)))
...
(ert-deftest dl-satan-ingest-cursor/backlog-depth-cursor-at-head-is-zero ()
  ...
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
      ...
      (dl-satan-ingest-cursor-test--write-segment
       today "browser"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T08:00:00+10:00")))
      (dl-satan-ingest-cursor-test--write-articles
       content
       (list (dl-satan-ingest-cursor-test--article "2026-06-10T03:00:00.000Z")))
...
(ert-deftest dl-satan-ingest-cursor/backlog-depth-missing-cursor-full-count ()
  "backlog-depth with no cursor state returns full count per source (from-head)."
  (dl-satan-ingest-cursor-test--with-env
    (let ((today dl-satan-tools-activity-dir)
          (content dl-satan-tools-content-dir))
      (should-not (dl-satan-ingest-cursor-read)) ; confirm absent
      (dl-satan-ingest-cursor-test--write-segment
       today "focus"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T09:00:00+10:00")
             (dl-satan-ingest-cursor-test--seg "2026-06-10T09:30:00+10:00")))
      (dl-satan-ingest-cursor-test--write-segment
       today "browser"
       (list (dl-satan-ingest-cursor-test--seg "2026-06-10T08:00:00+10:00")))
      (dl-satan-ingest-cursor-test--write-articles
       content
       (list (dl-satan-ingest-cursor-test--article "2026-06-10T03:00:00.000Z")
             (dl-satan-ingest-cursor-test--article "2026-06-10T04:00:00.000Z")
             (dl-satan-ingest-cursor-test--article "2026-06-10T05:00:00.000Z")))
      (let ((depth (dl-satan-ingest-cursor-backlog-depth)))
        (should (= 2 (plist-get depth :focus)))
        (should (= 1 (plist-get depth :browser)))
...
(provide 'dl-satan-ingest-cursor-test)
;;; dl-satan-ingest-cursor-test.el ends here
