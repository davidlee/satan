;;; satan-patch-classify.el --- patch-vs-dispatch classifier -*- lexical-binding: t; -*-
...
;; Public surface:
;;
;;   (satan-patch-classify DIRECTIVE) -> 'patch | 'dispatch
;;   (satan-patch-classify-explain DIRECTIVE) -> (CLASS . REASON)

(require 'cl-lib)
(require 'subr-x)

(defconst satan-patch-classify--patch-verbs
...
(defconst satan-patch-classify--dispatch-hints
  '("read" "reads" "reading"
    "scan" "scans" "scanning"
    "show" "shows" "showing"
    "list" "lists" "listing"
...
(defun satan-patch-classify--words (text)
  "Return TEXT's word tokens lowercased."
  (split-string (downcase (or text "")) "\\W+" t))

(defun satan-patch-classify-explain (directive)
...
or a fallback string."
  (let* ((words (satan-patch-classify--words directive))
         (patch-hit
          (cl-find-if (lambda (w)
                        (member w satan-patch-classify--patch-verbs))
                      words))
         (dispatch-hit
          (cl-find-if (lambda (w)
                        (member w satan-patch-classify--dispatch-hints))
                      words)))
    (cond
...
(defun satan-patch-classify (directive)
  "Return `patch' or `dispatch' for DIRECTIVE.  See
`satan-patch-classify-explain' for the matching keyword."
  (car (satan-patch-classify-explain directive)))

(provide 'satan-patch-classify)
;;; satan-patch-classify.el ends here
