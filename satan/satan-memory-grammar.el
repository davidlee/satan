;;; satan-memory-grammar.el --- closed-world grammar + v1 seed -*- lexical-binding: t; -*-
...
;; sync test (`test/satan-memory-grammar-test.el') verifies that the
...
(defconst satan-memory-grammar-closed-values
  '((surface
...
(defconst satan-memory-grammar-aliases
...
(defconst satan-memory-grammar-default-weights
  '((project            . 1)
    (surface            . 1)
    (app                . 1)
    (mode               . 1)
...
(defun satan-memory-grammar--ns-symbol (ns)
  (if (symbolp ns) ns (intern ns)))

(defun satan-memory-grammar-namespace-world (ns)
  "Return `closed' or `open' for NS (symbol or string), or nil."
  (cdr (assq (satan-memory-grammar--ns-symbol ns)
             satan-memory-grammar-namespaces)))

(defun satan-memory-grammar-closed-values (ns)
  "Return list of legal values for closed-world NS, or nil."
  (cdr (assq (satan-memory-grammar--ns-symbol ns)
             satan-memory-grammar-closed-values)))

(defun satan-memory-grammar-alias-target (alias)
  "Return canonical handle string for ALIAS, or nil."
  (cdr (assoc alias satan-memory-grammar-aliases)))

(defun satan-memory-grammar-default-weight (ns)
  "Return default weight for NS (symbol or string), or nil."
  (cdr (assq (satan-memory-grammar--ns-symbol ns)
             satan-memory-grammar-default-weights)))

(defun satan-memory-grammar-valid-value-p (ns value)
...
upstream in the canonicalizer)."
  (let ((world (satan-memory-grammar-namespace-world ns)))
    (pcase world
      ('closed (and (stringp value)
                    (member value (satan-memory-grammar-closed-values ns))
                    t))
      ('open   (and (stringp value) (> (length value) 0)))
      (_ nil))))

(provide 'satan-memory-grammar)
;;; satan-memory-grammar.el ends here
