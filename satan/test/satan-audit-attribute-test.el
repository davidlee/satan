;;; satan-audit-attribute-test.el --- attribute audit-event validator -*- lexical-binding: t; -*-
...
;;     -l satan-audit-attribute-test.el -f ert-run-tests-batch-and-exit

(require 'ert)
(require 'cl-lib)
(require 'satan-audit)

(defconst satan-audit-attr-test--run-id
  "20260523T120000-morning-deadbe")

(defconst satan-audit-attr-test--iv-id
...
(ert-deftest satan-audit-attribute/accepts-canonical-contradicted ()
  (should-not
   (satan-audit-validate-attribute-event
    "attribute.delta_applied"
    (satan-audit-attr-test--delta-applied))))

(ert-deftest satan-audit-attribute/accepts-worked-with-tiny-shame-delta ()
...
(ert-deftest satan-audit-attribute/accepts-each-outcome-reason ()
...
(ert-deftest satan-audit-attribute/accepts-each-attribute-name ()
  (dolist (name '("curiosity" "hunger" "suspicion" "doubt"
                  "friction" "shame" "brooding" "metamorphosis"))
    (should-not
     (satan-audit-validate-attribute-event
      "attribute.delta_applied"
      (satan-audit-attr-test--delta-applied :name name)))))

(ert-deftest satan-audit-attribute/accepts-disabled-true ()
  (should-not
   (satan-audit-validate-attribute-event
    "attribute.delta_applied"
    (satan-audit-attr-test--delta-applied :disabled t))))

(ert-deftest satan-audit-attribute/accepts-caps-applied ()
  (should-not
   (satan-audit-validate-attribute-event
    "attribute.delta_applied"
    (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/rejects-unknown-event ()
...
(ert-deftest satan-audit-attribute/rejects-unknown-source ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :source "telepathy"))))
    (should (stringp err))
    (should (string-match-p "unknown source" err))))

(ert-deftest satan-audit-attribute/rejects-reserved-unimplemented-source ()
  ...
  (dolist (src '("percept" "resonance" "tool_error" "manual"))
    (let ((err (satan-audit-validate-attribute-event
                "attribute.delta_applied"
                (satan-audit-attr-test--delta-applied :source src))))
      (should (stringp err))
      (should (string-match-p "reserved but unimplemented" err)))))

(ert-deftest satan-audit-attribute/rejects-unknown-attribute-name ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :name "rage"))))
    (should (stringp err))
    (should (string-match-p "field name must be one of" err))))

(ert-deftest satan-audit-attribute/rejects-unknown-scope ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :scope "episode"))))
    (should (stringp err))
    (should (string-match-p "field scope must be one of" err))))

(ert-deftest satan-audit-attribute/rejects-bad-source-reason-pairing ()
  ;; "shame" is not a valid reason for source=outcome.
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :reason "shame"))))
    (should (stringp err))
    (should (string-match-p "not valid for source=" err))))

(ert-deftest satan-audit-attribute/rejects-unknown-cap-name ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/rejects-old-out-of-range ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/rejects-new-out-of-range ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/rejects-delta-mismatch ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/accepts-delta-with-float-epsilon ()
...
(ert-deftest satan-audit-attribute/rejects-missing-id ()
  (let* ((payload (satan-audit-attr-test--delta-applied))
         (without-id (cl-loop for (k v) on payload by #'cddr
                              unless (eq k :id)
                              append (list k v)))
         (err (satan-audit-validate-attribute-event
               "attribute.delta_applied" without-id)))
    (should (stringp err))
    (should (string-match-p "missing required field: id" err))))

(ert-deftest satan-audit-attribute/rejects-missing-disabled ()
  (let* ((payload (satan-audit-attr-test--delta-applied))
...
(ert-deftest satan-audit-attribute/rejects-missing-evidence-confidence ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
               :evidence (list :intervention_id satan-audit-attr-test--iv-id
...
(ert-deftest satan-audit-attribute/rejects-missing-evidence-intervention-id ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
...
(ert-deftest satan-audit-attribute/rejects-missing-evidence-classification ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
               :evidence (list :intervention_id satan-audit-attr-test--iv-id
...
(ert-deftest satan-audit-attribute/rejects-bad-evidence-confidence ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
               :evidence (list :intervention_id satan-audit-attr-test--iv-id
...
(ert-deftest satan-audit-attribute/rejects-bad-evidence-classification ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied
               :evidence (list :intervention_id satan-audit-attr-test--iv-id
...
(ert-deftest satan-audit-attribute/rejects-bad-disabled-type ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :disabled "no"))))
    (should (stringp err))
    (should (string-match-p "field disabled must be boolean" err))))

(ert-deftest satan-audit-attribute/rejects-non-array-caps-applied ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--delta-applied :caps_applied "range_clamp"))))
    (should (stringp err))
...
(ert-deftest satan-audit-attribute/accepts-hippocampus-written ()
  (should-not
   (satan-audit-validate-attribute-event
    "attribute.delta_applied"
    (satan-audit-attr-test--hippocampus-delta))))

(ert-deftest satan-audit-attribute/accepts-each-hippocampus-reason ()
  (dolist (reason '("written" "overwritten" "deleted" "renamed" "searched"))
    (should-not
     (satan-audit-validate-attribute-event
      "attribute.delta_applied"
      (satan-audit-attr-test--hippocampus-delta :reason reason)))))

(ert-deftest satan-audit-attribute/rejects-unknown-hippocampus-reason ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--hippocampus-delta :reason "pondered"))))
    (should (stringp err))
    (should (string-match-p "not valid for source=" err))))

(ert-deftest satan-audit-attribute/rejects-hippocampus-missing-tool-name ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--hippocampus-delta
...
(ert-deftest satan-audit-attribute/rejects-hippocampus-missing-filename ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
              (satan-audit-attr-test--hippocampus-delta
               :evidence (list :tool_name "hippocampus_write")))))
...
(ert-deftest satan-audit-attribute/rejects-outcome-reason-for-hippocampus ()
  (let ((err (satan-audit-validate-attribute-event
              "attribute.delta_applied"
...
(provide 'satan-audit-attribute-test)
;;; satan-audit-attribute-test.el ends here
