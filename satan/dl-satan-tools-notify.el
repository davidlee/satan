...
(defconst dl-satan-notify-intervention-window-minutes 30
  "Default `outcome_window_minutes' for notify interventions (outcome-semantics §3.3).")

(defun dl-satan-notify--severity-for-urgency (urgency)
...
(defun dl-satan-tool/notify-send (args ctx)
...
Side effects:
  ...
  - Emits `intervention.created' into the run's transcript and INSERTs
    the row into `satan_interventions' via `dl-satan-intervention-create'.
      ...
      (condition-case err
          (let ((notify-id (notifications-notify
            ...
            (let ((iv-id (dl-satan-intervention-create
                          ...
                          dl-satan-notify-intervention-window-minutes
                          :severity (dl-satan-notify--severity-for-urgency urgency))))
