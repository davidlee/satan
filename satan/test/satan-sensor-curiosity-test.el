;;; satan-sensor-curiosity-test.el --- ert tests for satan-sensor-curiosity -*- lexical-binding: t; -*-
...
(defmacro satan-sensor-curiosity-test--with-fixture (state-var seg-var &rest body)
...
`satan-attribute-updates-enabled' so probes are live.  Cleaned up on exit."
  (declare (indent 2))
  `(let* ((,state-var (make-temp-file "satan-sensor-curiosity-state-"))
          (,seg-var (make-temp-file "satan-sensor-curiosity-seg-" t))
          (satan-sensor-curiosity-state-file ,state-var)
          (satan-sensor-curiosity-segments-dir ,seg-var)
...
(defun satan-sensor-curiosity-test--seed-state (path watermark)
  "Write a state file at PATH with :last_inspected WATERMARK."
  (with-temp-file path
    (insert (json-serialize (list :last_inspected watermark)))))

(defun satan-sensor-curiosity-test--read-watermark (path)
...
(defun satan-sensor-curiosity-test--write-segments (seg-dir rows)
...
(ert-deftest satan-sensor-curiosity/read-snapshot-charges-nothing ()
...
watermark.  The forbidden writers are spied to fail if called."
  (satan-sensor-curiosity-test--with-fixture state-path seg-dir
    (satan-sensor-curiosity-test--seed-state state-path "2026-06-09T08:00:00+10:00")
    (satan-sensor-curiosity-test--write-segments
    ...
    (cl-letf (((symbol-function 'satan-attribute-enqueue)
      ...
      (let ((snap (satan-sensor-curiosity-probe-read
        ...
        (should (equal (satan-sensor-curiosity-test--read-watermark state-path)
                       "2026-06-09T08:00:00+10:00"))))))

;; --- commit advances to native high-water (the bugfix) ---------

(ert-deftest satan-sensor-curiosity/commit-advances-to-max-end-ts-not-ts ()
  ...
  (satan-sensor-curiosity-test--with-fixture state-path seg-dir
    ...
    (cl-letf (((symbol-function 'satan-attribute-enqueue)
      ...
      (let* ((ts "2026-06-09T12:34:56+10:00") ; broker wall-clock — must NOT win
             (snap (satan-sensor-curiosity-probe-read :run-id "rid" :ts ts)))
        (should (plist-get snap :emit))
        ...
        (should (satan-sensor-curiosity-probe-commit snap))
        (let ((wm (satan-sensor-curiosity-test--read-watermark state-path)))
...
(ert-deftest satan-sensor-curiosity/read-without-commit-keeps-backlog ()
...
backlog — no signal is lost."
  (satan-sensor-curiosity-test--with-fixture state-path seg-dir
    ...
    (cl-letf (((symbol-function 'satan-attribute-enqueue)
      ...
      (let ((snap1 (satan-sensor-curiosity-probe-read
                    :run-id "rid" :ts "2026-06-09T09:30:00+10:00")))
      ...
      (let ((snap2 (satan-sensor-curiosity-probe-read
        ...
        (should (satan-sensor-curiosity-probe-commit snap2))
        (should (equal (satan-sensor-curiosity-test--read-watermark state-path)
                       "2026-06-09T09:00:00+10:00"))))))

(provide 'satan-sensor-curiosity-test)
;;; satan-sensor-curiosity-test.el ends here
